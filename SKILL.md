...
10. When using server mode or BOM upload features, keep `CDXGEN_ALLOWED_HOSTS` and related host allowlists narrow. Wildcard entries are for real subdomains only (for example, `*.example.com`), not suffix matches.
