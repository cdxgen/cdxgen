...
| ASTGEN_IGNORE_FILE_PATTERN       | Regular expression used to skip files during JavaScript/TypeScript AST generation. Defaults to `(?<!vite\.|vue\.)(conf\|config\|test\|spec\|mock\|setup-jest\|\.d)\.(js\|ts\|tsx)$`, which skips most config, test, spec, mock, and declaration files while still processing `vite.config.*` and `vue.config.*` files so that Vite and Vue CLI build-tool packages are correctly scoped. |
