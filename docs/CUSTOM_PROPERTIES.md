...
| `cdx:mcp:*`                                                                                | JavaScript / TypeScript MCP source inventory                             | MCP SDK provenance, server transport/auth posture, registered tools/prompts/resources, explicit model/provider literals | Detect unauthenticated MCP HTTP exposure, prioritize non-official MCP wrappers for review, and inventory the server primitives exposed to models                | [Cross-cutting BOM/service/build metadata](#inventory-cross-cutting) | [5](#example-5)                  |
