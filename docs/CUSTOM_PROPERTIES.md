...
| `cdx:golem:callGraphMode`                        | Shows whether Go Evinse used none, static, RTA, or pointer call graph analysis                  | Warning / triage |
...
| `cdx:golem:callGraphMode`                 | metadata.component    | enum string            | `static`, `rta`, `pointer`                             | On the root metadata component after Go Evinse runs Golem                                            | Explains precision and performance trade-offs for the evidence run                              | Context only     |
...
`cdx:golem:*` properties are emitted by `evinse -l go` when the `golem` helper is available. They are intentionally split between the root metadata component and dependency components.

Use metadata-level values to judge the evidence run: tool version, call graph mode, file/module counts, generated file counts, native artifact counts, security signal totals, build directive kinds, Go directive version, toolchain directive, and workspace/vendor presence.

Use component-level values to judge dependency posture: module path, Go version, usage scopes, occurrence evidence kinds, direct import status, symbol usage kind, local replacement status, vendoring, private module hints, license-file evidence, and security signal category/severity.

These properties are safe for policy because they are small values, counts, categories, and paths. They do not copy Go generator commands, environment values, embedded file contents, or secrets into the BOM. Source file names and private module paths can still reveal internal structure, so review an enriched BOM before sharing it outside the organization.
