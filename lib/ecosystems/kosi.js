...
/**
 * Orchestrates one kosi analyze run over src and returns the parsed report.
 * The analysis is read-only and offline: kosi never executes the project's
 * build. A classpath.txt beside the sources (the documented build-tool
 * report) is passed through when present.
 *
 * @param {string} src Directory to analyze.
 * @param {Object} options Configuration options.
 * @returns {Object|undefined} Parsed kosi report or undefined (fallback).
 */
export function analyzeKosiProject(src, options = {}) {
  ...
  const outputFile = join(tempDir, "kosi.json");
  const args = [
    ...kosi.args,
    "analyze",
    "--backend", "resolved",
    "--dataflow", "all",
    "--endpoint-sources",
    "--sarif-out", join(tempDir, "kosi.sarif"),
    "--dir", resolve(src),
    "--out", outputFile,
  ];
  ...
  if (DEBUG_MODE) {
    console.log("Executing", kosi.cmd, args.join(" "));
  }
  try {
 ...
 */
function addUsageEvidence(kosiReport, purlAliasMap, purlLocationMap, dataFlowFrames) {
 ...
 */
function addDataFlowEvidence(kosiReport, purlAliasMap, purlLocationMap, dataFlowFrames, componentPropertiesMap) {
  ...
  for (const slice of dataFlow.slices || []) {
    ...
    for (const nodeId of slice.nodeIds || []) {
      ...
      const resolvedPurl = resolveComponentPurl(node?.purl, purlAliasMap);
      if (resolvedPurl) {
        purls.add(resolvedPurl);
      }
    }
...
export function collectKosiEvidence(kosiReport = {}, components = []) {
  ...
  addUsageEvidence(kosiReport, purlAliasMap, purlLocationMap, dataFlowFrames);
  addDataFlowEvidence(kosiReport, purlAliasMap, purlLocationMap, dataFlowFrames, componentPropertiesMap);
