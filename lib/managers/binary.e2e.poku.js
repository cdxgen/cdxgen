...
await it("getOSPackages() end-to-end on debian rootfs surfaces dpkg capabilities", async () => {
  ...
  try {
    runCommand("bash", [
      "-lc",
      `cat > ${quoteForShell(path.join(emptyPluginsDir, "plugins-manifest.json"))} <<'EOF'
${JSON.stringify(
  {
    plugins: [
      {
...
EOF`,
    ...
    process.env.CDXGEN_PLUGINS_DIR = emptyPluginsDir;
    process.env.TRIVY_CMD = trivyBinary;
    process.env.TRUSTINSPECTOR_CMD = trustInspectorBinary;
    const { getOSPackages } = await importBinaryModule();
    const result = await getOSPackages(rootfsDir, {
      Env: [
        "PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
    ...
    assert.ok(
      result.osPackages.some((component) =>
        (component.properties || []).some((property) =>
          property.name.endsWith("PackageMaintainer"),
        ),
      ),
    );
    assert.ok(
      ...
      result.osPackages.some(
        (component) =>
          ...
          (component.cryptoProperties?.assetType === "certificate" ||
            (component.properties || []).some(
              (property) => property.name === "cdx:crypto:sourceType",
            )),
      ),
    );
  } finally {
