...
function loadPluginManifest() {
  ...
  try {
    pluginManifest = JSON.parse(
      readFileSync(PLUGIN_MANIFEST_FILE, { encoding: "utf-8" }),
    );
  } catch (_err) {
    pluginManifest = null;
  }
      ...
      if (tmpBom?.components) {
        for (const comp of tmpBom.components) {
          if (comp.purl) {
            ...
            if (trivyMetadata.retainedProperties.length) {
              comp.properties = trivyMetadata.retainedProperties;
            } else {
...
function collectTrustInspectorRootfsInventory(basePath) {
  const trustData = executeTrustInspector(["rootfs", basePath], {
    target: basePath,
  });
...
function isPpaRepository(url) {
  ...
  return (
    lowerUrl.includes("ppa.launchpadcontent.net") ||
    lowerUrl.includes("ppa.launchpad.net")
  );
}
...
async function createOSPackageFileComponents(basePath, osPackageEntries) {
  ...
  for (const packageEntry of osPackageEntries) {
    ...
    for (const filePath of packageEntry.files || []) {
      ...
      if (!fileComponent) {
        fileComponent = await createOSPackageFileComponent(basePath, filePath);
        if (!fileComponent) {
          continue;
        }
...
async function createOSPackageFileComponent(basePath, filePath) {
  ...
  const fileType = determineOwnedFileType(normalizedFilePath, stats);
...
function determineOwnedFileType(filePath, stats) {
  if (!stats) {
    return "file";
  }
...
export function enrichOSComponentsWithTrustData(components = []) {
  ...
  const pathInspectionCandidates = uniqueSortedStrings(
    components
      .flatMap((component) => extractComponentHostPaths(component))
      .slice(0, 250),
  );
  if (pathInspectionCandidates.length) {
    const inspectionData = executeTrustInspector(
      ["paths", ...pathInspectionCandidates],
      { target: pathInspectionCandidates.join(",") },
    );
    const inspectionMap = new Map(
      (inspectionData?.inspections || []).map((inspection) => [
        inspection.path,
