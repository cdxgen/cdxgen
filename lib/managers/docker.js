...
const getDefaultOptions = (forRegistry, requestedRegistryRef = forRegistry) => {
  ...
  const authDecisionTarget =
    requestedRegistryRef || normalizedForRegistry || forRegistry || DOCKER_HUB_REGISTRY;
  const opts = {
  ...
  if (
    ...
    authTokenSet = true;
    noteCredentialSource("DOCKER_USER/DOCKER_PASSWORD/DOCKER_EMAIL", "selected");
        ...
        if (configJson.auths) {
          ...
          for (const serverAddress of Object.keys(configJson.auths)) {
            ...
            if (configJson.auths[serverAddress].auth) {
              ...
              authTokenSet = true;
              noteCredentialSource("docker-config-auth", "selected", serverAddress);
              break;
            }
  ...
  if (!dockerHost) {
    ...
    if (dockerCertPath) {
      ...
      opts.https = {
        certificate: readFileSync(dockerCertFile, "utf8"),
        key: readFileSync(dockerKeyFile, "utf8"),
      };
  ...
  if (!selectedCredentialSource) {
    noteCredentialSource("anonymous", "selected", "no credential source resolved");
  }
