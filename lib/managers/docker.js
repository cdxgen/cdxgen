...
const DEFAULT_REGISTRY_PORTS = new Set(["80", "443"]);
...
const normalizeRegistryPath = (registryPath) => {
  ...
  normalizedPath = normalizedPath.replace(/\/v[12]$/i, "");
...
const buildRegistryAuthority = (hostname, port) => {
  ...
  if (port && !DEFAULT_REGISTRY_PORTS.has(port)) {
...
const parseRegistryReference = (registry) => {
  ...
  if (registry.includes("://")) {
    ...
    return {
      authority: buildRegistryAuthority(registryUrl.hostname, registryUrl.port),
  ...
  try {
    const registryUrl = new URL(`https://${authority}`);
    return {
      authority: buildRegistryAuthority(registryUrl.hostname, registryUrl.port),
...
export const makeRequest = async (
  ...
  method,
  forRegistry,
  requestedRegistryRef = forRegistry,
) => {
  if (isDryRun) {
    recordActivity({
      kind: "network",
  ...
  const defaultOptions = getDefaultOptions(forRegistry, requestedRegistryRef);
  const opts = {
