...
const normalizeRegistryHost = (registry) => {
  if (!registry?.trim()) {
    return undefined;
  }
  registry = registry.trim().toLowerCase();
  if (registry.includes("://") && URL.canParse(registry)) {
...
const registriesMatch = (configuredRegistry, requestedRegistry) => {
  ...
  const normalizedConfiguredRegistry =
    normalizeRegistryHost(configuredRegistry);
  const normalizedRequestedRegistry = normalizeRegistryHost(requestedRegistry);
  if (!normalizedConfiguredRegistry || !normalizedRequestedRegistry) {
    return false;
  }
  if (normalizedConfiguredRegistry === normalizedRequestedRegistry) {
    return true;
  ...
  return (
    DOCKER_HUB_REGISTRY_ALIASES.has(normalizedConfiguredRegistry) &&
    DOCKER_HUB_REGISTRY_ALIASES.has(normalizedRequestedRegistry)
  );
};
...
const getDefaultOptions = (forRegistry) => {
  ...
  forRegistry = normalizeRegistryHost(forRegistry);
  const opts = {
      ...
      try {
        ...
        if (configJson.auths) {
          ...
          for (const serverAddress of Object.keys(configJson.auths)) {
            if (!registriesMatch(serverAddress, forRegistry)) {
              continue;
            }
...
export const getCredsFromHelper = (exeSuffix, serverAddress) => {
  const cacheKey = `${exeSuffix}:${normalizeRegistryHost(serverAddress) ?? serverAddress}`;
