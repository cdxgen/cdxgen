...
await it("docker makeRequest accepts normalized exact matches across ipv4 ipv6 ports default ports and scoped subpaths from config auths", async () => {
  const cases = [
    {
...
await it("docker makeRequest rejects wildcard unicode bidi port-boundary and unrelated scoped-path mismatches from config auths", async () => {
...
await it("docker makeRequest accepts ipv4 ipv6 default-port and scoped-subpath registry matches from credHelpers", async () => {
  const cases = [
    {
      configuredRegistry: "127.0.0.1:5000",
      requestedRegistry: "127.0.0.1:5000/team/app",
...
await it("docker makeRequest does not invoke credHelpers for wildcard unicode bidi or port-boundary mismatches", async () => {
  const bidiRegistry = "reg\u202eistry.example.com";
  const unicodeConfusableRegistry = "reg\u0456stry.example.com";
  const cases = [
    {
