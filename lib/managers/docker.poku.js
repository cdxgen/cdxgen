...
await it("docker makeRequest accepts normalized exact matches across ipv4 ipv6 ports and subpaths from config auths", async () => {
  const cases = [
    {
...
await it("docker makeRequest rejects wildcard unicode bidi and port-boundary registry mismatches from config auths", async () => {
...
await it("docker makeRequest accepts ipv4 and ipv6 registry matches from credHelpers", async () => {
  const cases = [
    {
      configuredRegistry: "127.0.0.1:5000",
      requestedRegistry: "127.0.0.1:5000/team/app",
