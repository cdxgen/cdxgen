...
it("createDaemonConnection() talks to a unix socket daemon", async () => {
...
it("createDaemonConnection() forwards default headers", async () => {
  const daemon = await startUnixDaemon();
  const conn = createDaemonConnection(daemon.prefixUrl, {
...
it("createDaemonConnection() streams response bodies", async () => {
  const daemon = await startUnixDaemon();
  const conn = createDaemonConnection(daemon.prefixUrl);
  try {
    const chunks = [];
...
it("createDaemonConnection() throws on daemon error status codes", async () => {
  const daemon = await startUnixDaemon();
  const conn = createDaemonConnection(daemon.prefixUrl);
  try {
