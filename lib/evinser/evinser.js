...
export async function analyzeProject(dbObjMap, options) {
  ...
  if (isKosiKotlinLanguage(language)) {
    ...
    if (kosiReports && kosiReports.report && Object.keys(kosiReports.report).length) {
      ...
      // app's own flows) would have nothing to attach to. Supply the
      // application component from kosi's module discovery when the BOM
      // lacks one — never overwriting a component cdxgen did emit.
      if (!bomJson.metadata?.component) {
        const workspaceModule = (kosiReports.report.modules || []).find(
          (candidate) => candidate.workspaceMember !== undefined,
        );
      ...
      const analysisComponents = [bomJson.metadata?.component, ...components];
      const kosiEvidence = collectKosiEvidence(kosiReports.report, analysisComponents);
      mergeKosiEvidence(kosiEvidence, collectKosiEvidence(kosiReports.reachableReport || {}, analysisComponents));
