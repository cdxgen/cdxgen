...
export async function analyzeProject(dbObjMap, options) {
  ...
  if (isKosiKotlinLanguage(language)) {
    const kosiReport = analyzeKosiProject(dirPath, options);
    if (kosiReport && Object.keys(kosiReport).length) {
      const kosiEvidence = collectKosiEvidence(kosiReport, [
        bomJson.metadata?.component,
        ...components,
      ]);
      servicesMap = collectKosiServices(kosiReport, servicesMap);
      return {
