...
describe("reporter dispatcher", () => {
  ...
  it("annotations reporter returns the BOM with annotations appended", () => {
    ...
    assert.ok(first.subjects.includes(bomJson.serialNumber));
    assert.ok(first.properties.some((p) => p.name === "cdx:validate:ruleId"));
    assert.ok(first.annotator);
  });

  it("annotations reporter skips when spec version is below 1.4", () => {
    const bomJson = {
      bomFormat: "CycloneDX",
      specVersion: "1.3",
    ...
    assert.strictEqual(ann.length, 0);
  });
});
