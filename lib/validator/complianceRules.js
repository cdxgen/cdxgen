...
function inventoryComponents(bomJson) {
  ...
  return bomJson.components.filter((c) =>
    [
      "application",
      "framework",
      "library",
 ...
 * or an expression built from SPDX operators. This is a best-effort check: we
 * require either a license id on the component, an SPDX-style expression, or
 * a recognised composite ("AND", "OR", "WITH", parentheses).
 *
 * @param {string} expr
 * @returns {boolean}
 */
function looksLikeSpdx(expr) {
  ...
  // Accept either a single identifier (alphanumeric + -, ., +) or an SPDX
  // expression with AND/OR/WITH operators.
  return /^[A-Za-z0-9.+\-/():\s]+(?:\s+(?:AND|OR|WITH)\s+[A-Za-z0-9.+\-/():\s]+)*$/.test(
    expr,
  );
}
...
const SCVS_RULES = [
  ...
  },
  {
    ...
    automatable: true,
    evaluate(bomJson) {
      const comps = Array.isArray(bomJson?.components)
        ? bomJson.components
        : [];
  ...
  },
  {
    ...
    description:
      "Components defined in SBOM have valid SPDX license ID's or expressions (if applicable).",
    standard: "SCVS",
      ...
      if (invalid.length) {
        return fail(
          `${invalid.length} component(s) use a non-SPDX license expression.`,
          {
            ...
            locations: invalid.slice(0, 25).map(({ comp, lic }) => ({
              bomRef: comp["bom-ref"],
              purl: comp.purl,
              evidence: lic,
            })),
...
const CRA_RULES = [
  ...
  },
  {
    ...
    evaluate(bomJson) {
      ...
      return pass(
        `${deps.length} dependency relationship node(s) covering ${comps.length} component(s).`,
      );
    },
  },
  {
