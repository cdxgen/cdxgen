...
import { normalizeCycloneDxSpecVersion } from "../helpers/bomUtils.js";
...
export const validateBom = (bomJson) => {
  ...
  const specVersion = normalizeCycloneDxSpecVersion(bomJson.specVersion);
  let validate;
  if (specVersion && specVersion >= 2) {
    const schema = JSON.parse(
      readFileSync(
        join(dirName, "data", "cyclonedx-2.0-bundled.schema.json"),
        "utf-8",
      ),
    );
    ...
    addFormats(ajv);
    const bundledCryptoDefSchema = { ...cryptoDefSchema };
    delete bundledCryptoDefSchema.$schema;
    bundledCryptoDefSchema.$id =
      "https://cyclonedx.org/schema/cryptography-defs.schema.json";
    ajv.addSchema(bundledCryptoDefSchema);
    const bundledJsfSchema = { ...jsfSchema };
    delete bundledJsfSchema.$schema;
    bundledJsfSchema.$id = "https://cyclonedx.org/schema/jsf-0.82.schema.json";
    ajv.addSchema(bundledJsfSchema);
    const bundledSpdxSchema = { ...spdxSchema };
    delete bundledSpdxSchema.$schema;
    bundledSpdxSchema.$id = "https://cyclonedx.org/schema/spdx.schema.json";
    ajv.addSchema(bundledSpdxSchema);
