...
const BUNDLED_SPDX_MODEL = JSON.parse(
...
export const validateSpdx = (spdxJson) => {
  ...
  if (!Array.isArray(BUNDLED_SPDX_MODEL) || !BUNDLED_SPDX_MODEL.length) {
  ...
  for (const element of spdxJson?.["@graph"] || []) {
    ...
    switch (element.type) {
      ...
      case "SpdxDocument":
        ...
        break;
      case "Relationship":
        if (!element.creationInfo || !element.from) {
          errorList.push(
  ...
  for (const element of spdxJson?.["@graph"] || []) {
    ...
    if (element.from && !knownRefs.has(element.from)) {
      errorList.push(
