...
const GGUF_FILENAME_REGEX =
  /^(?:(?<Sidecar>mmproj|mtp)-)?(?<BaseName>[A-Za-z0-9\s]*(?:(?:-(?:(?:[A-Za-z\s][A-Za-z0-9\s]*)|(?:[0-9\s]*)))*))-(?:(?<SizeLabel>(?:\d+x)?(?:\d+\.)?\d+[A-Za-z](?:-[A-Za-z]+(?:\d+\.)?\d+[A-Za-z]+)?)(?:-(?<FineTune>[A-Za-z0-9\s-]+))?)?-(?:(?<Version>v\d+(?:\.\d+)*))(?:-(?<Encoding>(?!LoRA|vocab)[\w_]+))?(?:-(?<Type>LoRA|vocab))?(?:-(?<Shard>\d{5}-of-\d{5}))?\.gguf$/u;
...
const readLengthValue = (dataView, state, label) => {
...
const readGgufValue = (dataView, state, type) => {
  switch (type) {
    ...
    case GGUF_METADATA_TYPES.STRING: {
      const length = readLengthValue(dataView, state, "string length");
    ...
    case GGUF_METADATA_TYPES.ARRAY: {
      ...
      state.offset += 4;
      const length = readLengthValue(dataView, state, "array length");
      const values = [];
...
export function parseGgufFilename(filePathOrName) {
  ...
  const match = GGUF_FILENAME_REGEX.exec(fileName);
  if (!match?.groups) {
    return undefined;
  }
  const parsed = {
  ...
  if (match.groups.Sidecar) {
    parsed.sidecar = match.groups.Sidecar;
  }
  if (match.groups.BaseName) {
    parsed.baseName = match.groups.BaseName;
  }
  if (match.groups.SizeLabel) {
    parsed.sizeLabel = match.groups.SizeLabel;
  }
  if (match.groups.FineTune) {
    parsed.fineTune = match.groups.FineTune;
  }
  if (match.groups.Version) {
    parsed.version = match.groups.Version;
  }
  if (match.groups.Encoding) {
    parsed.encoding = match.groups.Encoding;
  }
  if (match.groups.Type) {
    parsed.type = match.groups.Type;
  }
  if (match.groups.Shard) {
    parsed.shard = match.groups.Shard;
    const shardParts = /^(\d{5})-of-(\d{5})$/u.exec(match.groups.Shard);
    if (shardParts) {
      parsed.shardIndex = Number.parseInt(shardParts[1], 10);
      parsed.shardCount = Number.parseInt(shardParts[2], 10);
    }
  }
  return parsed;
}
...
export function parseGgufMetadataBuffer(buffer) {
  ...
  const metadataCount = readLengthValue(dataView, state, "metadata count");
  const metadata = {
