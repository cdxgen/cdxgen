...
const createMalformedStringLengthBuffer = (stringLength) => {
  const chunks = [];
  const pushU32 = (value) => {
  ...
  pushU64(0);
  pushU64(1);
  pushString("general.name");
  pushU32(GGUF_METADATA_TYPES.STRING);
  pushU64(stringLength);
  return Buffer.concat(chunks);
};

const createMalformedValueTypeBuffer = (valueType) => {
  const chunks = [];
  const pushU32 = (value) => {
    const buffer = Buffer.allocUnsafe(4);
    buffer.writeUInt32LE(value, 0);
...
describe("GGUF parser", () => {
  ...
  it("rejects oversized GGUF metadata string lengths", () => {
    assert.throws(
      () => parseGgufMetadataBuffer(createMalformedStringLengthBuffer(1048577)),
      /exceeds allowed limit/u,
    );
  });

  it("rejects unsupported GGUF metadata types", () => {
    assert.throws(
      () => parseGgufMetadataBuffer(createMalformedValueTypeBuffer(256)),
      /Unsupported GGUF metadata type 256/u,
    );
  });
});
