...
describe("parseNpmrcFromEnv - Security", () => {
  ...
  test("records npm auth environment reads during dry-run tracing", () => {
    setDryRunMode(true);
    resetRecordedActivities();
    try {
      parseNpmrcFromEnv({
        NPM_CONFIG_//registry.example.com/:_authToken: "secret-token",
      });
      const activities = getRecordedActivities().filter(
        (activity) =>
          activity.kind === "env" &&
          activity.target ===
            "env:NPM_CONFIG_//registry.example.com/:_authToken",
      );
      assert.strictEqual(activities.length, 1);
      assert.match(activities[0].reason, /sensitive environment variable/);
    } finally {
      setDryRunMode(false);
      resetRecordedActivities();
    }
