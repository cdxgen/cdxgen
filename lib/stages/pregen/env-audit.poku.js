...
describe("auditEnvironment - Credential Exposure", () => {
  ...
  test("should NOT flag common system vars with credential-like substrings mid-name", () => {
    ...
    assert.ok(
      !warnings.some((w) => w.variable === "SSH_AUTH_SOCK"),
      "SSH_AUTH_SOCK should not be flagged (ends with _SOCK, not _AUTH)",
    );
    assert.ok(
      !warnings.some((w) => w.variable === "__CF_USER_TEXT_ENCODING"),
      "__CF_USER_TEXT_ENCODING should not be flagged (ends with _ENCODING, not _USER)",
    );
  });
