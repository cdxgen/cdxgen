...
describe("auditEnvironment - Deno Permissions", () => {
  ...
  test("should flag permission-misuse when Deno shell execution is broadly granted (Unix)", () => {
    globalThis.Deno = {
      ...
      permissions: {
        querySync: (desc) => {
          if (desc.name === "run" && ["sh", "bash"].includes(desc.command)) {
    ...
    globalThis.Deno = {
      ...
      permissions: {
        querySync: (desc) => {
          if (
            desc.name === "run" &&
            ["cmd", "powershell"].includes(desc.command)
          ) {
            return { state: "granted" };
          }
          return { state: "prompt" };
  ...
  test("should NOT flag permission-misuse when Deno restricts shell execution", () => {
    globalThis.Deno = {
      build: { os: "linux" },
      permissions: {
        // Only npm and node are allowed; sh/bash are not
        querySync: (desc) => {
          if (desc.name === "run" && ["npm", "node"].includes(desc.command)) {
            return { state: "granted" };
          }
          return { state: "denied" };
        },
