...
export function displaySelfThreatModel(
    ...
    for (const key of sensitive) {
      if (config[key] || config[toCamel(key)]) {
        addRisk(
          ...
          `Config file sets '${key}', which affects SBOM content or remote submission behaviour.`,
          "configuration",
        );
      }
    }
  ...
  if (
    options.output &&
    !options.output.startsWith(filePath) &&
    !options.output.startsWith("./") &&
    !options.output.startsWith(".")
  ) {
    addRisk(
      "medium",
      `Output path '${options.output}' is outside the project directory. Ensure write access to that location is intentional.`,
      "configuration",
    );
  }
