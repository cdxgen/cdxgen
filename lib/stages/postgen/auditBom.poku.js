...
import { setDryRunMode } from "../../helpers/utils.js";
import {
  auditBom,
  formatAnnotations,
...
describe("auditBom", () => {
  ...
  it("attaches dry-run support summary metadata to findings in dry-run mode", async () => {
    setDryRunMode(true);
    try {
      const findings = await auditBom(makeBom([]), {
        bomAuditCategories: "package-integrity",
      });
      assert.deepStrictEqual(findings.dryRunSupportSummary, {
        fullCount: 10,
        noCount: 3,
        partialCount: 1,
        totalRules: 14,
