...
describe("formatConsoleOutput", () => {
  it("should return empty string for no findings", () => {
    assert.strictEqual(formatConsoleOutput([], {}), "");
    assert.strictEqual(formatConsoleOutput(null, {}), "");
  });
  ...
  it("should format findings with severity icons", () => {
    const findings = [
      ...
      },
      {
        ...
        severity: "medium",
        category: "ci-permission",
        message: "Test medium finding",
      },
    ];
    const output = formatConsoleOutput(findings, {});
    assert.ok(output.includes("🔴"), "Should contain high severity icon");
    assert.ok(output.includes("🟡"), "Should contain medium severity icon");
    assert.ok(output.includes("CI-001"), "Should contain rule ID");
  });
});
...
describe("formatAnnotations", () => {
  it("should create CycloneDX annotations from findings", () => {
    ...
    assert.strictEqual(annotations[0].text, "Unpinned GitHub Action detected");
    assert.ok(
      annotations[0].properties,
      "Annotations should include properties",
    );
    assert.ok(
...
describe("hasCriticalFindings", () => {
  ...
  it("should respect custom fail severity", () => {
