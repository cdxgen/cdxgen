...
describe("evaluateRule", () => {
  ...
  it("should detect npm install script from direct manifest source (PKG-001)", async () => {
    ...
    const bom = makeBom([
      makeComponent("sketchy-pkg", "1.0.0", [
        ...
        ["cdx:npm:manifestSource", "git+https://github.com/acme/sketchy-pkg.git"],
      ]),
    ]);
  ...
  it("should detect python dependency from direct manifest source (PKG-009)", async () => {
    const rules = await loadRules(RULES_DIR);
    ...
    const bom = makeBom([
      makeComponent("suspicious-python-pkg", "1.0.0", [
        ...
        ["cdx:pypi:manifestSource", "https://example.com/suspicious-python-pkg.whl"],
      ]),
    ]);
