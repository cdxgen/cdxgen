...
describe("additional OBOM and rootfs hardening rules", () => {
  ...
  it("should ignore unknown notarization assessments for Apple-managed system apps (OBOM-MAC-007)", async () => {
    const rules = await loadRules(RULES_DIR);
    const rule = rules.find((r) => r.id === "OBOM-MAC-007");
    assert.ok(rule, "OBOM-MAC-007 rule should exist");
  ...
  it("should keep unknown notarization findings for user-controlled macOS launch agents (OBOM-MAC-007)", async () => {
    const rules = await loadRules(RULES_DIR);
    const rule = rules.find((r) => r.id === "OBOM-MAC-007");
    assert.ok(rule, "OBOM-MAC-007 rule should exist");
  ...
  it("should render actionable Windows Authenticode findings with registration and target paths (OBOM-WIN-012)", async () => {
    const rules = await loadRules(RULES_DIR);
    const rule = rules.find((r) => r.id === "OBOM-WIN-012");
    assert.ok(rule, "OBOM-WIN-012 rule should exist");
