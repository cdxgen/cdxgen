...
describe("loadRules", () => {
  ...
  it("should load rules with all required fields", async () => {
    ...
    const golemComplianceRules = rules.filter(
      (r) => r.category === "golem-compliance",
    );
    assert.ok(
      golemComplianceRules.length > 0,
      "Should have Golem compliance rules",
    );
  });
...
describe("evaluateRule", () => {
  ...
  it("should detect Golem native or generated build surfaces (GOLEM-PERF-002)", async () => {
    const rules = await loadRules(RULES_DIR);
