          ...
          properties: [
            {
              ...
              value: "https://user:pass@example.com/mcp?access_token=abc123456789",
            },
          ],
          type: "application",
        },
...
it("postProcess allows sensitive property values when TLP is strong", () => {
  ...
  const result = postProcess(bomNSData, { failOnError: true, specVersion: 1.7 });
  assert.strictEqual(
...
it("postProcess does not enforce TLP validation when no TLP is set", () => {
  ...
  const result = postProcess(bomNSData, { failOnError: true, specVersion: 1.7 });
  assert.strictEqual(result.bomJson.metadata.distributionConstraints, undefined);
});
