...
const SPDX_CDX_PROPERTIES_EXTENSION_TYPE =
  "https://spdx.org/rdf/3.0.1/terms/Extension/CdxPropertiesExtension";
const SPDX_CDX_PROPERTY_ENTRY_TYPE =
...
const toSpdxExternalReferences = (component) => {
  const references = toReferenceList(component?.externalReferences);
  const homepageReference = references.find((reference) =>
    ["website", "documentation", "distribution", "release-notes"].includes(
      reference?.type,
    ),
  );
  const downloadReference = references.find((reference) =>
    ["distribution", "download"].includes(reference?.type),
  );
  return {
    references,
    homepageReference,
...
const appendExtensionPropertyEntries = (propertyEntries, key, value) => {
  if (value === undefined || value === null) {
    return;
  }
  if (
    Array.isArray(value) &&
    value.every((entry) => entry?.name && isPrimitiveValue(entry?.value))
  ) {
    for (const entry of value) {
      propertyEntries.push({
        type: SPDX_CDX_PROPERTY_ENTRY_TYPE,
        cdxPropName: `${key}.${entry.name}`,
        cdxPropValue: `${entry.value}`,
      });
    }
    return;
  }
  propertyEntries.push({
    ...
    cdxPropName: key,
    cdxPropValue:
      typeof value === "string" ? value : JSON.stringify(toSerializableValue(value)),
  });
};
...
const toSpdxExtensions = (extensionData) => {
  ...
  for (const [key, value] of Object.entries(extensionData)) {
    appendExtensionPropertyEntries(propertyEntries, key, value);
  }
...
const toSpdxPackage = (component, creationInfoId, spdxId) => {
  ...
  if (
    externalReferenceData.homepageReference?.url &&
    component?.type !== "file"
  ) {
    spdxPackage.software_homePage = externalReferenceData.homepageReference.url;
  }
  if (
    externalReferenceData.downloadReference?.url &&
    component?.type !== "file"
  ) {
  ...
  if (externalReferenceData.references.length) {
    spdxPackage.externalRefs = externalReferenceData.references;
  }
...
export function convertCycloneDxToSpdx(bomJson, options = {}) {
  ...
  if (
    documentExtensions ||
    graphElements.some((element) => Array.isArray(element?.[SPDX_EXTENSION_KEY]))
  ) {
    spdxDocument.profileConformance = [
      ...SPDX_DOCUMENT_PROFILES,
      SPDX_EXTENSION_PROFILE,
    ];
  }
  return {
