...
const toSpdxExternalRefList = (referenceInput) => {
  ...
  for (const reference of toArray(referenceInput)) {
    ...
    const spdxReference = {
      externalRefType:
        SPDX_EXTERNAL_REF_TYPE_MAP[reference?.type] || "other",
...
const toSpdxExternalReferences = (component) => {
  ...
  const homepageReference = references.find((reference) =>
    ["altWebPage", "documentation", "altDownloadLocation", "releaseNotes"].includes(
      reference?.externalRefType,
    ),
  );
  const downloadReference = references.find((reference) =>
    reference?.externalRefType === "altDownloadLocation",
  );
  return {
    references,
    homepageReference,
