...
function filterTypedComponentCollections(
  ...
  subject.components = filterComponentArrayByType(
    subject.components,
    allowedTypes,
  );
  if (includeMetadata && isObjectRecord(subject.metadata)) {
    subject.metadata.component = filterComponentObjectByType(
      subject.metadata.component,
      allowedTypes,
    );
    if (isObjectRecord(subject.metadata.tools)) {
      subject.metadata.tools.components = filterComponentArrayByType(
        subject.metadata.tools.components,
        allowedTypes,
      );
  ...
  if (isObjectRecord(subject.definitions)) {
    subject.definitions.components = filterComponentArrayByType(
      subject.definitions.components,
      allowedTypes,
    );
  }
  if (Array.isArray(subject.vulnerabilities)) {
    for (const vulnerability of subject.vulnerabilities) {
      if (isObjectRecord(vulnerability?.tools)) {
        vulnerability.tools.components = filterComponentArrayByType(
