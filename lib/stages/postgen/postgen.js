...
import { matchesAiInventoryType } from "../../helpers/aiInventory.js";
...
export function filterBom(bomJson, options) {
  ...
  const excludeInventoryType = (subject) => {
    const excludedTypes = Array.isArray(options?.excludeType)
      ? options.excludeType
      : options?.excludeType
        ? [options.excludeType]
        : [];
    return excludedTypes.some(
      (type) =>
        ["ai-skill", "mcp", "skill", "skills"].includes(String(type)) &&
        matchesAiInventoryType(
          subject,
          ["skill", "skills"].includes(String(type))
            ? "ai-skill"
            : String(type),
        ),
    );
  ...
  for (const service of bomJson.services || []) {
    if (excludeInventoryType(service)) {
      filtered = true;
      continue;
    }
