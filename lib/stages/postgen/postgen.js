...
import { mergeDependencies } from "../../cli/index.js";
 ...
 */
function applyFormulation(bomJson, options, formulationList) {
  if (!options.includeFormulation || options.specVersion < 1.5 || !bomJson) {
    return bomJson;
  }
  // Skip if the BOM already has a formulation section (should not happen, but guard anyway)
  if (bomJson.formulation?.length) {
    return bomJson;
  }
  const context = formulationList?.length ? { formulationList } : {};
  const formulationData = addFormulationSection(options, context);
  if (!formulationData) {
    return bomJson;
  }
...
export function postProcess(bomNSData, options) {
  ...
  // Attach the formulation section exactly once, after the final BOM is assembled.
