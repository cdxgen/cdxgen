...
function normalizeToolsForSpecVersion(subject, specVersion) {
  ...
  if (!subject.tools || typeof subject.tools !== "object") {
    return;
  }
...
function upgradeSubjectForSpecVersion(subject, specVersion, parentKey) {
  if (!subject || typeof subject !== "object") {
    return;
  }
  if (Array.isArray(subject)) {
    subject.forEach((entry) => {
      upgradeSubjectForSpecVersion(entry, specVersion, parentKey);
    });
    return;
  }
  if (parentKey === "metadata") {
    if (
      subject.manufacture &&
      !subject.component?.manufacturer &&
      subject.component
    ) {
      subject.component.manufacturer = subject.manufacture;
    }
    deleteFields(subject, METADATA_2_0_REMOVED_FIELDS);
    normalizeToolsForSpecVersion(subject, specVersion);
  }
  if (parentKey === "component" || parentKey === "components") {
    if (!subject.authors && subject.author) {
      subject.authors = authorStringToAuthors(subject.author);
    }
    deleteFields(subject, COMPONENT_2_0_REMOVED_FIELDS);
  }
  Object.entries(subject).forEach(([key, value]) => {
    upgradeSubjectForSpecVersion(value, specVersion, key);
  });
}
...
function applySpecVersionCompatibility(bomJson, options) {
  ...
  return setCycloneDxFormat(bomJson, specVersion);
}
