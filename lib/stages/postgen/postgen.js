...
function downgradeSubjectForSpecVersion(subject, specVersion) {
  if (!subject || typeof subject !== "object") {
    return;
  }
  if (Array.isArray(subject)) {
    subject.forEach((entry) => {
      downgradeSubjectForSpecVersion(entry, specVersion);
    });
    return;
  }
  if (specVersion < 1.6) {
  ...
  Object.values(subject).forEach((value) => {
    downgradeSubjectForSpecVersion(value, specVersion);
  });
}
