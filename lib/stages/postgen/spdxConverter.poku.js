...
function getExtensionPropertyMap(spdxElement) {
  ...
  for (const extension of spdxElement?.extension || []) {
    for (const entry of extension?.cdxProperty || []) {
      propertyMap.set(entry.cdxPropName, entry.cdxPropValue);
    }
  }
  return propertyMap;
}
...
describe("convertCycloneDxToSpdx", () => {
  ...
  it("preserves advanced CycloneDX data in SPDX extension fields", () => {
    ...
    const documentExtensionProperties = getExtensionPropertyMap(documentElement);
    assert.strictEqual(
      documentElement.profileConformance.includes("extension"),
      true,
    );
    assert.strictEqual(
      documentExtensionProperties.get("metadataProperties.cdx:bom:componentTypes"),
      "library",
    );
    assert.strictEqual(
