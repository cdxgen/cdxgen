...
describe("mcpConfigParser", () => {
  ...
  it("records credential exposure without embedding raw secret metadata", async () => {
    ...
    assert.strictEqual(getProp(service, "cdx:mcp:credentialIndicatorCount"), "2");
    assert.strictEqual(getProp(service, "cdx:mcp:credentialExposureFieldCount"), "2");
    assert.strictEqual(getProp(service, "cdx:mcp:credentialReferenceCount"), "1");
    assert.strictEqual(
