...
function syntaxForFile(filePath) {
  return filePath.toLowerCase().endsWith(".json") ? "json" : "text";
}

function extractServerMaps(config) {
  const serverMaps = [];
  for (const [key, value] of Object.entries(config || {})) {
    if (["mcpServers", "context_servers", "servers"].includes(key)) {
...
function detectInlineCredentialIndicators(value) {
  ...
  for (const [name, pattern] of INLINE_CREDENTIAL_PATTERNS) {
...
function inferTransport(serverConfig, endpoints) {
  const declaredTransport = String(serverConfig?.transport || "").toLowerCase();
...
function createServiceFromConfig(
  ...
  const providerNames = providerNamesForText(
    JSON.stringify({ args, command, endpoints, env: serverConfig?.env || {} }),
  );
  ...
  const authenticated =
    transport === "stdio"
      ...
      : authPosture.some((value) => value !== "none");
  return {
    "bom-ref": `urn:service:mcp:${sanitizeRefToken(serviceName)}:${sanitizeRefToken(version)}`,
    authenticated,
    endpoints,
    group: "mcp",
    name: serviceName,
