...
describe("annotationFormatter", () => {
  ...
  it("truncates oversized details and property values for readable annotations", () => {
    const text = buildAnnotationText(
      "message",
      [{ name: "cdx:long", value: "x".repeat(250) }],
      Array.from({ length: 20 }, (_, index) => `detail ${index}`),
    );

    assert.ok(text.includes("more details omitted"));
    assert.ok(text.includes("… |"));
    assert.ok(!text.includes("detail 19"));
  });
});
