...
function addScopedProperties(componentPropertiesMap, purl, scopeCounts = {}) {
  const scopes = Object.keys(scopeCounts).filter((scope) => scopeCounts[scope] > 0);
...
function addOccurrenceKindProperties(componentPropertiesMap, purl, kindCounts = {}) {
...
function addSupplyChainProperties(
  ...
  for (const directive of supplyChain.replaces || []) {
    appendUniqueProperty(metadataProperties, "cdx:golem:replaceModule", directive.modulePath);
    appendUniqueProperty(
      metadataProperties,
  ...
  for (const directive of supplyChain.excludes || []) {
    appendUniqueProperty(metadataProperties, "cdx:golem:excludeModule", directive.modulePath);
  }
  for (const module of supplyChain.modules || []) {
    ...
    addPropertyValue(componentPropertiesMap, purl, "cdx:golem:vendored", module.vendored);
    addPropertyValue(
      componentPropertiesMap,
      purl,
...
function addImportEvidence(
  ...
  for (const importUsage of golemReport.imports || []) {
    ...
    incrementNestedCount(scopeCountsMap, purl, importUsage.usageScope || "runtime");
