...
`,
  ...
  it("builds release notes for github wrapper", async () => {
    const { buildReleaseNotesFromGithub } = await esmock("./source.js", {
      "./utils.js": {
        cdxgenAgent: { get: sinon.stub() },
        DEBUG_MODE: false,
        fetchPomXmlAsJson: sinon.stub(),
    ...
    const releaseNotes = buildReleaseNotesFromGithub(undefined, {
      githubRepository: "cdxgen/cdxgen",
      releaseNotesCurrentTag: "v1.0.0",
    });
    assert.strictEqual(releaseNotes.title, "Release notes for v1.0.0");
  });

  it("builds release notes for gitlab wrapper", async () => {
    const { buildReleaseNotesFromGitlab } = await esmock("./source.js", {
      "./utils.js": {
        cdxgenAgent: { get: sinon.stub() },
        DEBUG_MODE: false,
        fetchPomXmlAsJson: sinon.stub(),
    ...
    const releaseNotes = buildReleaseNotesFromGitlab(undefined, {
      gitlabRepository: "cdxgen/cdxgen",
      releaseNotesCurrentTag: "v1.0.0",
    });
    assert.strictEqual(releaseNotes.title, "Release notes for v1.0.0");
  });
