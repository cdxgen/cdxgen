...
`,
  ...
  it("builds release notes from provided tags", async () => {
    const { buildCdxgenReleaseNotes } = await esmock("./source.js", {
      "./utils.js": {
        cdxgenAgent: { get: sinon.stub() },
        DEBUG_MODE: false,
        fetchPomXmlAsJson: sinon.stub(),
    ...
    const releaseNotes = buildCdxgenReleaseNotes(undefined, {
  ...
  it("auto-detects local tags and commit resolves using hardened git command", async () => {
    const safeSpawnSync = sinon.stub().callsFake((_cmd, args) => {
      ...
      if (args.includes("--pretty=format:%H%x09%s")) {
        return { status: 0, stdout: "abcdef123456\tFix parser bug\n", stderr: "" };
      }
    ...
    const releaseNotes = buildCdxgenReleaseNotes("/tmp/repo", {});
    ...
    assert.strictEqual(releaseNotes.resolves[0].type, "enhancement");
