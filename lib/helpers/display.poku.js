          ...
          getRecordedActivities: sinon.stub().returns([
            {
              ...
              status: "completed",
              target:
                "Image=ghcr.io/cdxgen/cdxgen, SrcFiles=Dockerfile,package.json,pnpm-lock.yaml",
            },
          ]),
          isDryRun: true,
          isSecureMode: false,
...
it("prints the activity summary as JSON", async () => {
  ...
  try {
    ...
    printActivitySummaryMocked("json");
    sinon.assert.calledOnce(consoleLogStub);
    const payload = JSON.parse(consoleLogStub.firstCall.args[0]);
...
it("prints the activity summary as JSON Lines", async () => {
  ...
  try {
    ...
    printActivitySummaryMocked("jsonl");
    sinon.assert.calledTwice(consoleLogStub);
    const summary = JSON.parse(consoleLogStub.firstCall.args[0]);
    const activity = JSON.parse(consoleLogStub.secondCall.args[0]);
