...
function sourceLocationFromDosaiNode(node) {
  const location = locationFromDosai(node);
...
function sourceLocationFromDosaiLocation(location) {
  const sourceLocation = locationFromDosai(location);
...
export function collectDosaiPurlEvidence(methodsSlice, components = []) {
  ...
  for (const dependency of methodsSlice?.Dependencies || []) {
    ...
    addSetValue(
      purlLocationMap,
      purl,
      sourceLocationFromDosaiLocation(dependency),
    );
    addSetValue(purlModulesMap, purl, dependency.Name || dependency.Namespace);
  }
  ...
  for (const reachability of methodsSlice?.PackageReachability || []) {
    ...
    for (const sourceLocation of reachability.SourceLocations || []) {
      const location = sourceLocationFromDosaiLocation(sourceLocation);
    ...
    for (const nodeId of reachability.NodeIds || []) {
      ...
      if (!hasExplicitSourceLocations) {
        addSetValue(purlLocationMap, purl, sourceLocationFromDosaiNode(node));
      }
