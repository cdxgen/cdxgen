...
const resolveStaticValue = (
  ...
  if (astNode.type === "Identifier") {
    if (stringLiteralByName.has(astNode.name)) {
      return stringLiteralByName.get(astNode.name);
    }
    if (numberLiteralByName.has(astNode.name)) {
      return numberLiteralByName.get(astNode.name);
    }
    if (objectLiteralByName.has(astNode.name)) {
      return objectLiteralByName.get(astNode.name);
    }
    return undefined;
  }
...
export function analyzeJsCryptoSource(source) {
  ...
  const numberLiteralByName = new Map();
  const objectLiteralByName = new Map();
  const stringLiteralByName = new Map();
  ...
  traverse.default(ast, {
    ...
    VariableDeclarator: (path) => {
      ...
      if (idNode.type === "Identifier") {
        if (typeof resolvedValue === "string") {
          stringLiteralByName.set(idNode.name, resolvedValue);
        } else if (typeof resolvedValue === "number") {
          numberLiteralByName.set(idNode.name, resolvedValue);
        } else if (resolvedValue && typeof resolvedValue === "object") {
          objectLiteralByName.set(idNode.name, resolvedValue);
        }
    ...
    CallExpression: (path) => {
      ...
      if (callee?.type === "Identifier") {
        ...
        const rootAlias = calleeParts[0];
        if (cryptoModuleAliases.has(rootAlias)) {
          nodeCryptoMethod = calleeParts[1];
        }
        if (
          jwtModuleAliases.has(rootAlias) &&
          JWT_METHOD_NAMES.has(calleeParts[1])
        ) {
          for (const argument of callNode.arguments || []) {
            const optionsValue = resolveStaticValue(
              argument,
