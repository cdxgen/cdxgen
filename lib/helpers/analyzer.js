...
const IGNORE_FILE_PATTERN = new RegExp(
  process.env.ASTGEN_IGNORE_FILE_PATTERN ||
    "(?<!vite\\.|vue\\.)(conf|config|test|spec|mock|setup-jest|\\.d)\\.(js|ts|tsx)$",
  "i",
);
...
const getVueConfigFiles = (
  ...
  for (const entry of files) {
    ...
    if (fileStat.isDirectory()) {
      ...
      if (deep) {
        getVueConfigFiles(file, deep, result, rootDir, excludePatterns);
      }
      continue;
    }
...
const parseVueConfigFiles = (
  ...
  const configFiles = getVueConfigFiles(src, deep, [], src, excludePatterns);
