...
const EXTENSION_CAPABILITY_CATEGORIES = [
  "fileAccess",
  "deviceAccess",
  "network",
  "bluetooth",
...
const EXTENSION_CAPABILITY_CHAIN_PATTERNS = {
  ...
  fingerprinting: [
    ...
    /^(canvas|getImageData|toDataURL|measureText)$/i,
  ],
};
...
const getMemberExpressionChain = (node) => {
  if (!node) {
    return "";
  }
  ...
  if (node.type === "CallExpression") {
    return getMemberExpressionChain(node.callee);
  }
  ...
  const objectChain = getMemberExpressionChain(node.object);
  const propertyChain = getMemberExpressionChain(node.property);
...
export const detectExtensionCapabilities = (src, deep = false) => {
  ...
  const addSignal = (rawSignal) => {
    ...
    for (const category of EXTENSION_CAPABILITY_CATEGORIES) {
      const categoryPatterns = EXTENSION_CAPABILITY_CHAIN_PATTERNS[category];
      if (categoryPatterns?.some((pattern) => pattern.test(signal))) {
  ...
  const addIdentifierSignal = (rawSignal) => {
    ...
    for (const category of Object.keys(
      EXTENSION_CAPABILITY_IDENTIFIER_PATTERNS,
    )) {
      const pattern = EXTENSION_CAPABILITY_IDENTIFIER_PATTERNS[category];
    ...
    try {
      ...
      traverse.default(ast, {
        MemberExpression: (path) => {
          addSignal(getMemberExpressionChain(path?.node));
        },
        OptionalMemberExpression: (path) => {
          addSignal(getMemberExpressionChain(path?.node));
        },
        CallExpression: (path) => {
          addSignal(getMemberExpressionChain(path?.node?.callee));
          if (path?.node?.callee?.type === "Identifier") {
        ...
        Identifier: (path) => {
          addIdentifierSignal(path?.node?.name);
        },
      });
    } catch (_err) {
