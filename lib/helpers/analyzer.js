...
const PYTHON_SERVER_CALL_TEMPLATE =
  "(\\w+)\\s*=\\s*ALIAS\\s*\\(([\\s\\S]{0,1600}?)\\n\\s*\\)";
const PYTHON_TOOLS_PATTERN = /mtypes\.Tool\s*\(([\s\S]{0,800}?)\)/gmu;
const PYTHON_PROMPTS_PATTERN = /mtypes\.Prompt\s*\(([\s\S]{0,800}?)\)/gmu;
const PYTHON_RESOURCES_PATTERN = /mtypes\.Resource\s*\(([\s\S]{0,800}?)\)/gmu;
...
export const detectPythonMcpInventory = (src, deep = false) => {
  ...
  for (const file of srcFiles) {
    ...
    for (const importEntry of importEntries) {
      ...
      recordMcpSdkImport(
        serviceInfo,
        sourceValue.startsWith("mcp") ? "mcp" : sourceValue,
      );
      if (
    ...
    for (const alias of serverConstructorAliases) {
      const constructorPattern = new RegExp(
        PYTHON_SERVER_CALL_TEMPLATE.replaceAll(
          "ALIAS",
          alias.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
        ),
        "gmu",
      );
      for (const match of raw.matchAll(constructorPattern)) {
        const serviceVarName = match[1];
        ...
        for (const decoratorMatch of raw.matchAll(PYTHON_DECORATOR_PATTERN)) {
          ...
          const primitiveRole = PYTHON_DECORATOR_ROLE_MAP.get(decoratorMatch[2]);
          if (!primitiveRole) {
            continue;
          }
    ...
    const primitivePatterns = [
      [PYTHON_TOOLS_PATTERN, "tool"],
      [PYTHON_PROMPTS_PATTERN, "prompt"],
      [PYTHON_RESOURCES_PATTERN, "resource"],
    ];
    for (const [pattern, role] of primitivePatterns) {
      for (const match of raw.matchAll(pattern)) {
        const serviceInfo = ensureMcpService(servicesByKey, file, fileRelativeLoc);
        registerPythonPrimitive(
          serviceInfo,
          role,
          extractPythonNamedString(match[1] || "", "name"),
          extractPythonNamedString(match[1] || "", "description"),
          extractPythonNamedString(match[1] || "", "uri"),
