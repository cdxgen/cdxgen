...
function analyzeJsCapabilitiesSource(source) {
  ...
  traverse.default(ast, {
    ...
    MemberExpression: (path) => {
      ...
      if (
        JS_FILE_ACCESS_CHAIN_PATTERNS.some((pattern) => pattern.test(memberChain))
      ) {
    ...
    NewExpression: (path) => {
      ...
      if (["WebSocket", "EventSource", "XMLHttpRequest"].includes(calleeChain)) {
