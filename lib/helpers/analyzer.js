...
const getStaticPrimitiveValue = (astNode) => {
  if (!astNode) {
    return undefined;
  }
...
const toResolvedValueArray = (value) => {
  if (value === undefined) {
    return [];
  }
...
const mergeResolvedValues = (...values) => {
  const mergedValues = [];
  const seen = new Set();
...
const getStaticObjectProperty = (objectValue, propertyName) => {
...
const getConditionComparisonValue = (astNode) => {
  return getStaticPrimitiveValue(astNode);
};

const deriveStaticNarrowingsFromCondition = (astNode, branchTaken) => {
  if (!astNode) {
    return undefined;
  }
  if (
    ...
    return deriveStaticNarrowingsFromCondition(astNode.expression, branchTaken);
  }
  if (astNode.type === "UnaryExpression" && astNode.operator === "!") {
    return deriveStaticNarrowingsFromCondition(astNode.argument, !branchTaken);
  }
  if (astNode.type === "LogicalExpression") {
    if (branchTaken && astNode.operator === "&&") {
      return mergeStaticNarrowingMapsForAnd(
        deriveStaticNarrowingsFromCondition(astNode.left, true),
        deriveStaticNarrowingsFromCondition(astNode.right, true),
      );
    }
    if (branchTaken && astNode.operator === "||") {
  ...
  const leftValue = getConditionComparisonValue(astNode.left);
  const rightValue = getConditionComparisonValue(astNode.right);
...
const resolveStaticValue = (astNode, staticValueByName, depth = 0) => {
  if (!astNode || depth > 6) {
    return undefined;
  }
  const primitiveValue = getStaticPrimitiveValue(astNode);
  ...
  if (
    ...
    return resolveStaticValue(astNode.expression, staticValueByName, depth + 1);
  }
...
const deriveStaticNarrowingsFromSwitchCase = (
  switchCaseNode,
  switchStatementNode,
  staticValueByName,
) => {
  ...
  if (!identifierName || switchCaseNode.test === null) {
    return undefined;
  }
  ...
  for (
    ...
    const caseValue = resolveStaticValue(caseNode.test, staticValueByName);
    if (
      !hasOnlyResolvedValues(
        caseValue,
        (candidate) =>
...
export function analyzeJsCryptoSource(source) {
  ...
  const getScopedStaticValueByName = (path) => {
    const scopedStaticValueByName = new Map(staticValueByName);
    const branchNarrowings = [];
    ...
    while (currentPath?.parentPath) {
      ...
      if (parentPath.node?.type === "IfStatement") {
        ...
        if (branchTaken !== undefined) {
          const narrowedValues = deriveStaticNarrowingsFromCondition(
            parentPath.node.test,
            branchTaken,
          );
          if (narrowedValues?.size) {
            branchNarrowings.push(narrowedValues);
          }
        }
      }
      if (
        ...
        const narrowedValues = deriveStaticNarrowingsFromSwitchCase(
          parentPath.node,
    ...
    branchNarrowings.reverse().forEach((narrowedValues) => {
  ...
  traverse.default(ast, {
    ...
    VariableDeclarator: (path) => {
      ...
      const scopedStaticValueByName = getScopedStaticValueByName(path);
      const resolvedValue = resolveStaticValue(
        initNode,
        scopedStaticValueByName,
      );
    ...
    CallExpression: (path) => {
      ...
      if (callee?.type === "Identifier") {
        ...
        if (jwtMethod && JWT_METHOD_NAMES.has(jwtMethod)) {
          for (const argument of callNode.arguments || []) {
            const optionsValue = resolveStaticValue(
              argument,
              scopedStaticValueByName,
            );
            ...
            for (const [
              propertyName,
              primitive,
            ] of CRYPTO_OBJECT_PROPERTY_KEYS) {
              const propertyValue = getStaticObjectProperty(
                optionsValue,
                propertyName,
              );
        ...
        if (calleeParts[calleeParts.length - 1] === "setProtectedHeader") {
          const headerValue = resolveStaticValue(
            callNode.arguments?.[0],
            scopedStaticValueByName,
          );
      ...
      if (
        ...
        if (
          ...
          const argumentIndex = ["hkdf", "hkdfSync"].includes(nodeCryptoMethod)
            ? 0
            : 0;
          const algorithmValue = resolveStaticValue(
        ...
        if (["pbkdf2", "pbkdf2Sync"].includes(nodeCryptoMethod)) {
          const digestValue = resolveStaticValue(
            callNode.arguments?.[4],
            scopedStaticValueByName,
          );
          recordCryptoAlgorithmsFromValue(
            algorithms,
        ...
        if (
          ...
          const typeValue = resolveStaticValue(
            callNode.arguments?.[0],
            scopedStaticValueByName,
          );
          const optionsValue = resolveStaticValue(
      ...
      if (webcryptoMethod && WEBCRYPTO_METHOD_PRIMITIVES.has(webcryptoMethod)) {
        ...
        const primaryAlgorithmValue = resolveStaticValue(
          callNode.arguments?.[0],
          scopedStaticValueByName,
        );
        recordCryptoAlgorithmsFromValue(
        ...
        if (webcryptoMethod === "deriveKey") {
          const derivedAlgorithmValue = resolveStaticValue(
