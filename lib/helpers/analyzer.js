...
const sanitizeMcpRefToken = (value) =>
...
const objectExpressionBooleanValue = (astNode, propertyName) => {
  const propertyValue = objectExpressionProperty(astNode, propertyName)?.value;
  if (!propertyValue) {
    return undefined;
  }
  if (propertyValue.type === "BooleanLiteral") {
    return propertyValue.value;
  }
  return undefined;
};

const arrayLiteralStringValues = (astNode) => {
  if (!astNode || astNode.type !== "ArrayExpression") {
    return [];
  }
  return (astNode.elements || [])
    .map((element) => getLiteralStringValue(element))
    .filter(Boolean);
};
...
const serviceObjectForMcp = (serviceInfo) => {
  ...
  addUniqueProperty(properties, "cdx:mcp:serviceType", "server");
  ...
  for (const capability of Array.from(serviceInfo.capabilities).sort()) {
    addUniqueProperty(
      properties,
      `cdx:mcp:capabilities:${capability}`,
      "true",
    );
  }
  for (const [flagName, flagValue] of Array.from(
    ...
    addUniqueProperty(properties, `cdx:mcp:capabilities:${flagName}`, flagValue);
  }
  ...
  addUniqueProperty(
    properties,
    "cdx:mcp:toolCount",
    String(serviceInfo.primitives.filter((item) => item.role === "tool").length),
  );
  addUniqueProperty(
    properties,
    "cdx:mcp:promptCount",
...
/**
 * Detect MCP server inventory from JavaScript/TypeScript source using Babel AST analysis.
 *
 * @param {string} src Path to the project source directory
 * @param {boolean} deep When true, includes node_modules and nested directories
      ...
      traverse.default(ast, {
        ImportDeclaration: (path) => {
          ...
          if (classification.isMcp) {
            ...
            const serviceInfo = ensureMcpService(servicesByKey, file, fileRelativeLoc);
        ...
        VariableDeclarator: (path) => {
          ...
          if (
            ...
            if (classification.isMcp) {
              ...
              for (const moduleName of getNamedImportsFromObjectPattern(idNode)) {
          ...
          if (
            ...
            const serviceInfo = ensureMcpService(servicesByKey, file, fileRelativeLoc);
            const serverDefinition = extractServerDefinition(initNode.arguments?.[0]);
            serviceInfo.name = serverDefinition.name || serviceInfo.name;
            serviceInfo.version = serverDefinition.version || serviceInfo.version;
            ...
            if (initNode.callee.name === "URL") {
              ...
              if (urlValue) {
                ...
                if (urlValue.includes("/mcp")) {
                  ...
                  serviceInfo.endpoints.add(urlValue);
        ...
        CallExpression: (path) => {
          ...
          if (callee.type === "Identifier" && authHelperAliases.has(callee.name)) {
          ...
          if (
            serviceInfo &&
            ["registerPrompt", "registerResource", "registerTool"].includes(methodName)
          ) {
            const primitive = {
              annotations: undefined,
              description: undefined,
              name: undefined,
              role: methodName === "registerTool"
                ? "tool"
            ...
            if (methodName === "registerTool") {
              ...
              if (primitive.uri?.includes("{") || primitive.uri?.includes("}")) {
          ...
          if (
            ...
            if (routePath?.includes("/mcp")) {
              serviceInfo.endpoints.add(routePath);
              serviceInfo.transports.add("streamable-http");
            }
            if (
            ...
            for (const arg of callNode.arguments || []) {
              if (arg?.type === "Identifier" && authHelperAliases.has(arg.name)) {
                serviceInfo.authenticated = true;
                serviceInfo.xTrustBoundary = true;
              }
              if (
