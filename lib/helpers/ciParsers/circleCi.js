 ...
 *
 * Instead of silently dropping duplicates (which loses information), identical
 * steps are disambiguated by appending a ` (N)` counter to the step name.
 * The first occurrence is always left unchanged.
 *
 * @param {Object[]} steps
 * @returns {Object[]|undefined}
 */
function disambiguateSteps(steps) {
  if (!steps?.length) {
    return undefined;
  }
...
function parseCircleCiFile(f, _options) {
  ...
  // Collect orbs as components (pkg:orb/<namespace>/<name>@<version>)
    ...
    for (const [orbAlias, orbRef] of Object.entries(yamlObj.orbs)) {
      if (typeof orbRef === "string") {
        ...
        components.push({
          ...
          name,
          version,
          purl: `pkg:circleci/${fullName}@${version}`,
          properties: [
