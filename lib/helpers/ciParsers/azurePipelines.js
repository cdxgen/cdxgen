...
function parseAzurePipelinesFile(f, _options) {
  ...
  for (const stage of stages) {
    ...
    if (stage.dependsOn) {
      const depOn = Array.isArray(stage.dependsOn)
        ? stage.dependsOn
        : [stage.dependsOn];
      stageProperties.push({
    ...
    for (const jobDef of jobs) {
      ...
      const jobRef = uuidv4();
      const steps = [];
      const jobProperties = [{ name: "cdx:azure:job:name", value: jobName }];
    ...
    tasks.push({
      ...
      taskTypes: ["build"],
      steps: undefined,
      tasks: stageTasks.length ? stageTasks : undefined,
      properties: stageProperties,
    });
    workflowDependsOn.push(stageRef);

    if (stageDependsOn.length) {
      dependencies.push({ ref: stageRef, dependsOn: stageDependsOn });
    }
  }
