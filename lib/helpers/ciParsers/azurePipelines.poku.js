...
describe("azurePipelinesParser", () => {
  ...
  it("parses azure-pipelines-matrix.yml: multi-stage pipeline extracted", () => {
    ...
    assert.ok(stageNames.includes("Validate"), "expected Validate stage");
  ...
  it("parses azure-pipelines-matrix.yml: stage dependsOn recorded in properties", () => {
    ...
    const testStage = result.workflows[0].tasks.find((t) => t.name === "Test");
    assert.ok(testStage, "Test stage must exist");
    const depProp = testStage.properties.find(
