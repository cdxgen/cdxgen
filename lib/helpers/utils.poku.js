...
it("converts cdxgen exclude globs to Atom-compatible regex", () => {
  ...
  assert.ok(regex.test("/repo/ignored/generated.js"));
  assert.ok(regex.test("src/app/fixtures/demo.py"));
  assert.ok(regex.test("tests/unit.spec.js"));
  assert.ok(!regex.test("src/app/main.js"));
});

it("builds Atom exclude environment for Python and JavaScript", () => {
  const originalAstgenIgnoreDirs = process.env.ASTGEN_IGNORE_DIRS;
  const originalChenPythonIgnoreDirs = process.env.CHEN_PYTHON_IGNORE_DIRS;
  try {
    delete process.env.ASTGEN_IGNORE_DIRS;
    delete process.env.CHEN_PYTHON_IGNORE_DIRS;
    const options = {
      exclude: ["**/ignored/**", "src/generated/**", "**/*.spec.js"],
    };
    const jsEnv = buildAtomCommandEnv(options, "javascript");
    assert.ok(jsEnv.ASTGEN_IGNORE_DIRS.includes("node_modules"));
    assert.ok(jsEnv.ASTGEN_IGNORE_DIRS.includes("ignored"));
    assert.ok(jsEnv.ASTGEN_IGNORE_DIRS.includes("generated"));
    assert.ok(!jsEnv.ASTGEN_IGNORE_DIRS.split(",").includes("src"));

    const pythonEnv = buildAtomCommandEnv(options, "python");
    assert.ok(pythonEnv.CHEN_PYTHON_IGNORE_DIRS.includes("docs"));
    assert.ok(pythonEnv.CHEN_PYTHON_IGNORE_DIRS.includes("ignored"));
    assert.ok(pythonEnv.CHEN_PYTHON_IGNORE_DIRS.includes("generated"));
  } finally {
    ...
    if (originalChenPythonIgnoreDirs === undefined) {
      delete process.env.CHEN_PYTHON_IGNORE_DIRS;
    } else {
      process.env.CHEN_PYTHON_IGNORE_DIRS = originalChenPythonIgnoreDirs;
    }
  }
});
