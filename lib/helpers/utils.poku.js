...
it("safeExtractArchive() records extracted byte and file deltas in debug mode", async () => {
  const originalDebugMode = process.env.CDXGEN_DEBUG_MODE;
  process.env.CDXGEN_DEBUG_MODE = "debug";
  try {
    const utilsModule = await esmock("./utils.js");
    ...
    assert.strictEqual(archiveActivity.extractedFileCount, 2);
    assert.strictEqual(archiveActivity.extractedBytes, 7);
...
describe("convertOSQueryResults", () => {
  ...
  it("collectExecutables() records followed symlink resolutions in dry-run mode", () => {
    if (process.platform === "win32") {
      return;
    }
    const tempDir = mkdtempSync(path.join(tmpdir(), "cdxgen-executables-"));
    ...
    try {
      ...
      const symlinkActivity = getRecordedActivities().find(
        (activity) =>
          activity.kind === "symlink-resolution" &&
          activity.resolvedPath === "usr/bin/which",
      );
      assert.ok(symlinkActivity);
      assert.strictEqual(symlinkActivity.target, "bin/which");
      assert.strictEqual(symlinkActivity.traceDetail, undefined);
      assert.match(symlinkActivity.reason, /Resolved executable candidate/);
    } finally {
      setDryRunMode(false);
      resetRecordedActivities();
      rmSync(tempDir, { recursive: true, force: true });
