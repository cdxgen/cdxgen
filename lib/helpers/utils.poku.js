...
it("safeWriteSync() honors explicit fs.write permission for output files", async () => {
  ...
  const hasPermissionStub = sinon.stub().callsFake((scope, requestedPath) => {
    return (
      scope === "fs.read" ||
      (scope === "fs.write" && requestedPath === outputFile)
    );
  });
...
it("cdxgenAgent records completed and failed network activity outcomes", async () => {
  ...
  process.env.CDXGEN_ALLOWED_HOSTS = "127.0.0.1";
  process.env.CDXGEN_DEBUG_MODE = "debug";
  process.env.CDXGEN_SECURE_MODE = "0";
  try {
    const {
      cdxgenAgent,
      getRecordedActivities: getRecordedActivitiesMocked,
      ...
      new URL(`./utils.js?network-activity-test=${Date.now()}`, import.meta.url)
    );
    ...
    await cdxgenAgent.get(successUrl);
    assert.strictEqual(getRecordedActivitiesMocked()[0].status, "completed");
    assert.strictEqual(getRecordedActivitiesMocked()[0].target, successUrl);

    resetRecordedActivitiesMocked();
    const failureUrl = `http://127.0.0.1:${port}/failure`;
    await assert.rejects(() => cdxgenAgent.get(failureUrl));
