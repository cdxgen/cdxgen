...
it("parseYarnLock names aliased dependencies after the real package", async () => {
  ...
  try {
    ...
    // Non-scoped alias resolves to the real package name.
    assert.ok(
    ...
    // Scoped alias resolves to the real scoped package name.
    assert.ok(
    ...
    // Plain npm: entries (not aliases) keep their own name.
    assert.ok(
    ...
    // "@npm%3A" in the patch locator must not be mistaken for an alias.
    assert.ok(
