...
it("builds global Atom exclude environment", () => {
  ...
  try {
    ...
    const ignoreDirs = env.CHEN_IGNORE_DIRS.split(",");
    assert.deepStrictEqual(Object.keys(env), ["CHEN_IGNORE_DIRS"]);
    assert.ok(ignoreDirs.includes("vendor"));
    assert.ok(ignoreDirs.includes("ignored"));
    assert.ok(ignoreDirs.includes("generated"));
    assert.ok(ignoreDirs.includes("noxfile.py"));
    assert.ok(!ignoreDirs.includes("src"));
  } finally {
