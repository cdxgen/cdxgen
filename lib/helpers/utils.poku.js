...
it("parse cargo virtual workspace with inherited package and dependency metadata", async () => {
  const tmpDir = mkdtempSync(path.join(tmpdir(), "cdxgen-cargo-workspace-"));
  const workspaceToml = path.join(tmpDir, "Cargo.toml");
  const memberDir = path.join(tmpDir, "crates", "member-a");
  const memberToml = path.join(memberDir, "Cargo.toml");
...
`,
  ...
  try {
    ...
    const memberPackage = depList.find((pkg) => pkg.name === "member-a");
    ...
    assert.ok(serdeDep);
    assert.strictEqual(serdeDep.version, "1.0.217");
    assert.strictEqual(
      serdeDep.properties.find(
        (property) => property.name === "cdx:cargo:workspaceDependency",
      )?.value,
