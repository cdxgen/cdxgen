...
it("cdxgenAgent reads CDXGEN_ALLOWED_HOSTS once per request", async () => {
  let setDryRunModeMocked;
  try {
    const mockedProcess = createMockedProcess({
      CDXGEN_ALLOWED_HOSTS: "example.com",
    });
    const {
      cdxgenAgent,
      getRecordedActivities: getRecordedActivitiesMocked,
    ...
    setDryRunModeMocked = mockedSetDryRunMode;
    const beforeRequestHook = cdxgenAgent.hooks.beforeRequest[0];
    ...
    const envActivities = getRecordedActivitiesMocked().filter(
