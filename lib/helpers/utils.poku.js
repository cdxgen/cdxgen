...
describe("convertOSQueryResults", () => {
  ...
  it("should omit purl for osquery data components while keeping a stable bom-ref", () => {
    ...
    assert.ok(
      components[0]["bom-ref"].startsWith(
        "osquery/authorized_keys_snapshot/data/root@ssh-ed25519#",
      ),
    );
    assert.ok(!components[0]["bom-ref"].includes("undefined"));
  });
  ...
  it("should add LOLBAS properties to suspicious windows osquery rows", () => {
    ...
    assert.ok(
      components[0]["bom-ref"].startsWith(
        "osquery/windows_run_keys/data/HKCU%5CSoftware%5CMicrosoft%5CWindows%5CCurrentVersion%5CRun%5CUpdater@unknown#",
      ),
    );
