...
it("parse collider lock data sanitizes origin metadata and tracks invalid wrap hashes", () => {
  const colliderLockData = parseColliderLockData(
    JSON.stringify({
      ...
      dependencies: {
        "unsafe-origin": {
          version: "1.0.0",
          wrap_hash: "sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd",
  ...
  assert.strictEqual(colliderLockData.pkgList.length, 2);
  const unsafeOrigin = colliderLockData.pkgList.find(
    (pkg) => pkg.name === "unsafe-origin",
  );
  const malformed = colliderLockData.pkgList.find((pkg) => pkg.name === "malformed");
  assert.deepStrictEqual(
    unsafeOrigin.properties.find((property) => property.name === "cdx:collider:origin")
      ?.value,
  ...
  assert.deepStrictEqual(malformed.properties.find((property) => property.name === "cdx:collider:hasWrapHash")?.value, "false");
  assert.deepStrictEqual(malformed.properties.find((property) => property.name === "cdx:collider:wrapHashInvalid")?.value, "true");
  assert.deepStrictEqual(
