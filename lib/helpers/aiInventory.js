...
export function summarizeAiInventory(inventory) {
  ...
  return {
    instructionCount: components.filter((component) =>
      AI_INSTRUCTION_FILE_KINDS.has(propertyValue(component, "cdx:file:kind")),
    ).length,
    mcpConfigCount: components.filter(
      (component) =>
        propertyValue(component, "cdx:file:kind") === MCP_CONFIG_FILE_KIND,
    ).length,
    mcpServiceCount: services.length,
    skillCount: components.filter(
      (component) =>
        propertyValue(component, "cdx:file:kind") === AI_SKILL_FILE_KIND,
    ).length,
  };
}
