...
export function extractExtensionCapabilities(pkg) {
  ...
  // Contributed features with security implications
  const contributes = pkg.contributes || {};
  const contributedFeatures = [];
  if (contributes.commands?.length) {
    contributedFeatures.push(`commands:${contributes.commands.length}`);
  }
  if (contributes.debuggers?.length) {
    contributedFeatures.push(`debuggers:${contributes.debuggers.length}`);
  }
  ...
  // Entry points - for analyzer and security review
  if (pkg.main) {
  ...
  // Lifecycle scripts (postinstall can execute arbitrary code)
...
export function toComponent(extInfo, ideName) {
  ...
  if (caps.extensionKind?.length) {
    props.push({
      name: "cdx:vscode-extension:extensionKind",
      value: caps.extensionKind.join(", "),
    });
  }
  if (caps.extensionDependencies?.length) {
    props.push({
      name: "cdx:vscode-extension:extensionDependencies",
      value: caps.extensionDependencies.join(", "),
    });
  }
  if (caps.extensionPack?.length) {
    props.push({
      name: "cdx:vscode-extension:extensionPack",
      value: caps.extensionPack.join(", "),
    });
  }
  if (caps.untrustedWorkspaces !== undefined) {
    ...
    props.push({
      ...
      value:
        typeof uws === "object" ? uws.supported || String(uws) : String(uws),
    });
  }
  if (caps.virtualWorkspaces !== undefined) {
    ...
    props.push({
      ...
      value:
        typeof vws === "object" ? vws.supported || String(vws) : String(vws),
    });
  }
...
export async function extractVsixToTempDir(vsixFile) {
  try {
    const tempDir = mkdtempSync(join(getTmpDir(), "vsix-deps-"));
...
export function cleanupTempDir(tempDir) {
  ...
  const dirToRemove = tempDir.endsWith("extension")
    ? join(tempDir, "..")
    : tempDir;
  try {
    if (dirToRemove.includes("vsix-deps-")) {
