...
export function parseVsixPackageJson(packageJsonData, srcPath) {
  try {
    ...
    if (packageJsonData.repository?.url) {
      pkg.externalReferences = [
        { type: "vcs", url: packageJsonData.repository.url },
      ];
    }
    return {
      ...
      platform: "", // FIXME: How do you find this value?
      srcPath,
      externalReferences: pkg.externalReferences,
...
export function toComponent(extInfo, ideName) {
  ...
  const component = {
    ...
    type: "application",
    externalReferences: extInfo.externalReferences,
  };
