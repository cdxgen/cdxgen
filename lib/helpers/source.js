...
/**
 * Build CycloneDX release notes for the cdxgen tool component.
 *
 ...
 */
export function buildCdxgenReleaseNotes(repoPath, options = {}) {
  ...
  if (repoPath && !maybeRemotePath(repoPath)) {
    const repoCheck = hardenedGitCommand(["rev-parse", "--is-inside-work-tree"], {
      cwd: repoPath,
    });
  ...
  if ((!currentTag || !previousTag) && remoteUrl && /github\.com[:/]/i.test(remoteUrl)) {
  ...
  let timestamp;
  if (localRepoAvailable) {
    const tsResult = hardenedGitCommand(["log", "-1", "--format=%cI", currentTag], {
      cwd: repoPath,
    });
  ...
  if (localRepoAvailable && previousTag) {
    ...
    if (logResult.status === 0 && logResult.stdout) {
      for (const line of logResult.stdout.toString().split("\n").slice(0, 50)) {
