...
export function resolvePurlSourceDirectory(srcDir, purlResolution) {
  ...
  while (
    ...
    if (queueIndex >= MAX_MONOREPO_DIRECTORIES) {
      break;
    }
...
export async function resolveGitUrlFromPurl(purlString) {
  ...
  try {
    ...
    if (DEBUG_MODE) {
      ...
      console.log("Unable to resolve repository URL for purl.", purlString);
      console.log("Purl parsing error details.", errorMessage);
    }
    return undefined;
  }
  ...
  try {
    ...
    if (DEBUG_MODE) {
      const errorMessage = err?.message || String(err);
      console.log("Unable to resolve repository URL for purl.", purlString);
      console.log("Purl lookup error details.", errorMessage);
    }
    return undefined;
  }
