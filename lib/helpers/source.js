...
export function gitClone(repoUrl, branch = null) {
  ...
  if (branch) {
    ...
    if (!isSafeGitRefName(firstBranchStr)) {
      console.log("Skipping branch clone: invalid branch name");
    } else {
...
export function resolvePurlSourceDirectory(srcDir, purlResolution) {
  ...
  while (queue.length && packageJsonCount < MAX_MONOREPO_PACKAGE_JSON_FILES) {
    if (queue.length > MAX_MONOREPO_DIRECTORIES) {
      break;
    }
...
/**
 * Extract and normalize SCM URL candidates from a Maven POM XML document.
 *
 * @param {string|undefined} pomXml Maven POM XML string
 * @returns {string|undefined} normalized repository URL
 */
function normalizeMavenScmUrl(pomXml) {
  if (!pomXml || typeof pomXml !== "string") {
    return undefined;
  }
  const scmBlockMatch = pomXml.match(/<scm\b[\s\S]*?<\/scm>/i);
  if (!scmBlockMatch?.[0]) {
    return undefined;
  }
  const scmBlock = scmBlockMatch[0];
  for (const fieldName of ["url", "connection", "developerConnection"]) {
    const fieldMatch = scmBlock.match(
      new RegExp(`<${fieldName}>([^<]+)</${fieldName}>`, "i"),
    );
    const normalized = normalizeRepositoryUrl(fieldMatch?.[1]?.trim());
    if (normalized) {
      return normalized;
    }
  }
    ...
    switch (purlObj.type) {
      ...
      case "maven": {
        ...
        registry = mavenCentralUrl.endsWith("/")
          ? mavenCentralUrl
          : `${mavenCentralUrl}/`;
        const groupPath = purlObj.namespace.replaceAll(".", "/");
        const endpoint = `${registry}${groupPath}/${purlObj.name}/${packageVersion}/${purlObj.name}-${packageVersion}.pom`;
        const res = await cdxgenAgent.get(endpoint, {
          responseType: "text",
        });
        repoUrl = normalizeMavenScmUrl(res.body);
        break;
      }
      case "composer": {
        const packagistUrl =
