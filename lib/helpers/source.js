...
export function findGitRefForPurlVersion(repoUrl, purlResolution) {
  ...
  if (purlType === "npm" && purlName) {
    const scopedName = purlNamespace
      ? `${purlNamespace.startsWith("@") ? purlNamespace : `@${purlNamespace}`}/${purlName}`
      : purlName;
    refCandidates.push(
  ...
  const filteredCandidates = [...new Set(refCandidates)].filter(
    (candidate) => candidate && !candidate.startsWith("-"),
  );
  const result = safeSpawnSync(
    "git",
    ["ls-remote", "--refs", "--tags", "--heads", repoUrl],
    {
      shell: false,
      env: {
        ...process.env,
...
export function resolvePurlSourceDirectory(srcDir, purlResolution) {
  ...
  if (purlNamespace) {
    packageNameCandidates.push(
      ...
      `${purlNamespace.startsWith("@") ? purlNamespace : `@${purlNamespace}`}/${purlResolution.name}`,
    );
  }
  ...
  while (queue.length && packageJsonCount <= 2000) {
    if (queue.length > maxDirectories) {
      break;
    }
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (
          uniquePackageNameCandidates.includes(pkgJson?.name) &&
          currentDir !== srcDir
        ) {
  ...
  const dedupedMatches = [...new Set(matches)];
 ...
 * - cargo  -> crates.io
 * - pub    -> pub.dev
 * - docker -> derived repository URL from namespace/name or qualifiers
...
export async function resolveGitUrlFromPurl(purlString) {
  ...
  try {
    switch (purlObj.type) {
      ...
      case "docker": {
        const dockerVcsUrl = purlObj.qualifiers?.vcs_url;
        const dockerDownloadUrl = purlObj.qualifiers?.download_url;
        const dockerNamespace = purlObj.namespace;
        const dockerName = purlObj.name;
        repoUrl =
          normalizeRepositoryUrl(dockerVcsUrl) ||
          normalizeDownloadRepositoryUrl(dockerDownloadUrl);
        if (!repoUrl && dockerNamespace === "library") {
          repoUrl = normalizeRepositoryUrl(
            `https://github.com/docker-library/${dockerName}`,
          );
        }
        if (!repoUrl && dockerNamespace) {
          repoUrl = normalizeRepositoryUrl(
            `https://github.com/${dockerNamespace}/${dockerName}`,
          );
        }
        break;
      }
...
export function cleanupSourceDir(srcDir) {
  if (srcDir?.startsWith(getTmpDir()) && fs.rmSync) {
