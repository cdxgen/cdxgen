  ...
  if (localRepoAvailable && previousTag) {
    ...
    if (logResult.status === 0 && logResult.stdout) {
      ...
      for (const line of logResult.stdout
        ...
        .split("\n")
        .filter(Boolean)
        .slice(0, MAX_RELEASE_NOTE_RESOLVES)) {
...
/**
 * Build release notes for GitHub repositories.
 *
 * @param {string|undefined} repoPath local repository path
 * @param {Object} options options carrying release notes hints
 * @returns {Object|undefined} releaseNotes object
 */
export function buildReleaseNotesFromGithub(repoPath, options = {}) {
  let releaseNotesGitUrl = options.releaseNotesGitUrl;
  if (!releaseNotesGitUrl && options.githubRepository) {
    releaseNotesGitUrl = `https://github.com/${options.githubRepository}`;
  }
  return buildReleaseNotesFromGit(repoPath, {
    ...options,
    releaseNotesGitUrl,
  });
}
...
/**
 * Build release notes for GitLab repositories.
 *
 * @param {string|undefined} repoPath local repository path
 * @param {Object} options options carrying release notes hints
 * @returns {Object|undefined} releaseNotes object
 */
export function buildReleaseNotesFromGitlab(repoPath, options = {}) {
  let releaseNotesGitUrl = options.releaseNotesGitUrl;
  if (!releaseNotesGitUrl && options.gitlabRepository) {
    const gitlabHost = options.gitlabHost || "https://gitlab.com";
    releaseNotesGitUrl = `${gitlabHost}/${options.gitlabRepository}`;
  }
  return buildReleaseNotesFromGit(repoPath, {
    ...options,
    releaseNotesGitUrl,
