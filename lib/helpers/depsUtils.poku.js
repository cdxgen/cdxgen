...
describe("filterInvalidCryptoComponents()", () => {
    ...
    const components = [
      {
        ...
        cryptoProperties: { assetType: "algorithm", oid: "2.16.840.1.101.3.4.2.1" },
      },
      {
        name: "unknown-algo",
        type: "cryptographic-asset",
    ...
    const components = [
      { name: "bad-cert", type: "cryptographic-asset" },
      {
        ...
        cryptoProperties: {
          ...
          algorithmProperties: { executionEnvironment: "unknown", implementationPlatform: "unknown" },
        },
      },
    ];
    const result = filterInvalidCryptoComponents(components);
    ...
    const components = [
      {
        ...
        cryptoProperties: {
          ...
          relatedCryptoMaterialProperties: { type: "public-key", id: "abc", state: "active" },
        },
      },
    ];
    const result = filterInvalidCryptoComponents(components);
  ...
  it("returns empty/falsy input unchanged", () => {
    ...
    assert.strictEqual(filterInvalidCryptoComponents(undefined), undefined);
    assert.strictEqual(filterInvalidCryptoComponents(null), null);
  });
});
