...
function mergeServiceProperties(existingProps = [], newProps = []) {
  ...
  for (const newProp of newProps) {
    if (
      !merged.find(
        (prop) => prop?.name === newProp?.name && prop?.value === newProp?.value,
      )
    ) {
      merged.push(newProp);
    }
  ...
  for (const service of combined) {
    ...
    if (!serviceMap.has(key)) {
      serviceMap.set(key, {
        ...
        endpoints: Array.isArray(service.endpoints)
          ? Array.from(new Set(service.endpoints))
          : service.endpoints,
    ...
    if (Array.isArray(service.endpoints) && service.endpoints.length) {
      existing.endpoints = Array.from(
        new Set([...(existing.endpoints || []), ...service.endpoints]),
      );
    }
    ...
    if (Array.isArray(service.services) && service.services.length) {
      existing.services = mergeServices(existing.services || [], service.services);
    }
  }
