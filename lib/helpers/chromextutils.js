...
import { detectExtensionCapabilities } from "./analyzer.js";
...
const CHROMIUM_CAPABILITY_NAMES = [
  "fileAccess",
  "deviceAccess",
  "network",
  "bluetooth",
...
function mergeCapabilitySignals(...capabilityMaps) {
  const merged = {};
  for (const capabilityName of CHROMIUM_CAPABILITY_NAMES) {
  ...
  try {
    ...
    return {
      ...
      braveMaybeBackground:
        manifest.MAYBE_background && typeof manifest.MAYBE_background === "object",
      bravePermissions: permissions.filter((permission) =>
        ["webDiscovery", "settingsPrivate"].includes(permission),
      ),
      capabilities,
      hasAutofill,
    };
...
function parseChromeExtensionFromManifestPath(manifestFile) {
  ...
  const codeCapabilityScan = detectExtensionCapabilities(extensionDir, false);
  const codeCapabilities = {};
  for (const capabilityName of CHROMIUM_CAPABILITY_NAMES) {
    codeCapabilities[capabilityName] = codeCapabilityScan.capabilities.includes(
      capabilityName,
    );
  }
  ...
  return {
    component: toComponent({
      ...
      webAccessibleResourceMatches: manifest?.webAccessibleResourceMatches || [],
      externallyConnectableMatches:
...
export function toComponent(extInfo) {
  ...
  if (extInfo.capabilities) {
    const capabilityNames = CHROMIUM_CAPABILITY_NAMES.filter(
  ...
  for (const browserDir of browserDirs) {
    ...
    for (const profileName of profiles) {
      ...
      for (const extensionEntry of extensionEntries) {
        ...
        const codeCapabilityScan = detectExtensionCapabilities(
          extensionDir,
          false,
        );
        const codeCapabilities = {};
