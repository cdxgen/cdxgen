...
it("builds global Atom and JavaScript astgen exclude environment", () => {
  ...
  try {
    ...
    assert.ok(new RegExp(env.CHEN_IGNORE_REGEX).test("src/foo.spec.js"));
    assert.ok(new RegExp(env.CHEN_IGNORE_REGEX).test("legacy/file.js"));
    assert.ok(
      new RegExp(env.ASTGEN_IGNORE_FILE_PATTERN).test("src/foo.spec.js"),
    );

    const pythonEnv = buildAtomCommandEnv(options, "python");
    assert.deepStrictEqual(Object.keys(pythonEnv).sort(), [
      "CHEN_IGNORE_DIRS",
      "CHEN_IGNORE_REGEX",
    ]);
  } finally {
    ...
    if (originalChenIgnoreRegex === undefined) {
      delete process.env.CHEN_IGNORE_REGEX;
    } else {
      process.env.CHEN_IGNORE_REGEX = originalChenIgnoreRegex;
    }
  }
});
