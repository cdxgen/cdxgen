      ...
      if (localPkgList && localPkgList.length === 1) {
        ...
        if (localMetadata && Object.keys(localMetadata).length) {
          ...
          if (localMetadata?.properties?.length) {
            pkg.properties = pkg.properties.concat(localMetadata.properties);
          }
          pkg.properties.push({
            name: "LocalNodeModulesPath",
            value: packagePath,
      ...
      if (step.uses) {
        ...
        if (lineNum >= 0 && lines[lineNum]) {
          ...
          if (commentMatch?.[1]) {
            ...
            // Because we determined the version only from the comment, our confidence is low.
            confidence = 0.3;
          }
        }
        const key = `${group}-${name}-${version}`;
        if (!keys_cache[key] && name && version) {
          ...
          // Even though our confidence on version is low, we have a medium confidence on the purl
          if (version !== tagOrCommit) {
