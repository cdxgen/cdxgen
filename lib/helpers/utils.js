...
export function safeSpawnSync(command, args, options) {
  ...
  if (command.includes("python") && (!args?.length || args[0] !== "-S")) {
    console.warn(
      "\x1b[1;35mExecuting python command without the '-S' argument is considered unsafe and could lead to code execution risks.\x1b[0m",
    );
    if (isSecureMode) {
      console.log(
        "This might be a bug in cdxgen. Please report this bug with an example repo.",
      );
    }
  }
