...
const LOCKFILE_ACTIVITY_HINTS = new Map([
  ["bun.lock", { classification: "lockfile", ecosystem: "bun", label: "Bun lockfile" }],
  ["cargo.lock", { classification: "lockfile", ecosystem: "cargo", label: "Cargo lockfile" }],
  ["composer.lock", { classification: "lockfile", ecosystem: "composer", label: "Composer lockfile" }],
  ["gemfile.lock", { classification: "lockfile", ecosystem: "rubygems", label: "Bundler lockfile" }],
  ["package-lock.json", { classification: "lockfile", ecosystem: "npm", label: "npm lockfile" }],
  ["packages.lock.json", { classification: "lockfile", ecosystem: "nuget", label: "NuGet lockfile" }],
  ["pdm.lock", { classification: "lockfile", ecosystem: "python", label: "PDM lockfile" }],
  ["pnpm-lock.yaml", { classification: "lockfile", ecosystem: "pnpm", label: "pnpm lockfile" }],
  ["poetry.lock", { classification: "lockfile", ecosystem: "python", label: "Poetry lockfile" }],
  ["podfile.lock", { classification: "lockfile", ecosystem: "cocoapods", label: "CocoaPods lockfile" }],
  ["pylock.toml", { classification: "lockfile", ecosystem: "python", label: "PEP 751 lockfile" }],
  ["uv.lock", { classification: "lockfile", ecosystem: "python", label: "uv lockfile" }],
  ["yarn.lock", { classification: "lockfile", ecosystem: "yarn", label: "Yarn lockfile" }],
]);
const MANIFEST_ACTIVITY_HINTS = new Map([
  ["cargo.toml", { classification: "manifest", ecosystem: "cargo", label: "Cargo manifest" }],
  ["composer.json", { classification: "manifest", ecosystem: "composer", label: "Composer manifest" }],
  ["gemfile", { classification: "manifest", ecosystem: "rubygems", label: "Gem manifest" }],
  ["package.json", { classification: "manifest", ecosystem: "npm", label: "package manifest" }],
  ["pom.xml", { classification: "manifest", ecosystem: "maven", label: "Maven manifest" }],
  ["pyproject.toml", { classification: "manifest", ecosystem: "python", label: "Python project manifest" }],
  ["requirements.txt", { classification: "manifest", ecosystem: "python", label: "Python requirements manifest" }],
  ["setup.py", { classification: "manifest", ecosystem: "python", label: "Python setup manifest" }],
]);
...
const KEY_FILE_EXTENSIONS = new Set([".key", ".jks", ".keystore", ".p12", ".pfx"]);
...
function classifyDiscoveryPattern(pattern) {
  const patternValue = Array.isArray(pattern) ? pattern.join(",") : String(pattern);
...
export function recordObservedActivity(kind, target, options = {}) {
  ...
  const traceKey =
    options.traceKey || `${kind}:${status}:${target}:${options.traceDetail || ""}`;
...
export async function safeExtractArchive(
  ...
  if (isDryRun) {
    recordActivity({
      ...
      kind,
      reason: `Dry run mode blocks ${kind} extraction from ${sourcePath} into ${targetPath}.`,
      status: "blocked",
...
export function safeSpawnSync(command, args, options) {
  const activityDescriptor = buildCommandActivityDescriptor(command, args, options);
  const commandAllowed = isAllowedCommand(command);
  if (process.env.CDXGEN_ALLOWED_COMMANDS) {
    recordPolicyActivity(command, {
      metadata: {
        allowed: commandAllowed,
        allowlist: process.env.CDXGEN_ALLOWED_COMMANDS,
...
function inferNetworkIntent(requestUrl) {
  ...
  if (
    ...
    hostname.includes("spdx.org") ||
    hostname.includes("opensource.org")
  ) {
    return "license-fetch";
  }
  if (
    hostname.includes("registry.npmjs.org") ||
    hostname.includes("pypi.org") ||
    hostname.includes("rubygems.org") ||
    hostname.includes("repo.maven.apache.org") ||
    hostname.includes("repo1.maven.org") ||
    hostname.includes("crates.io") ||
    hostname.includes("pub.dev") ||
    hostname.includes("nuget.org")
  ) {
    return "registry-lookup";
  }
  if (hostname.includes("github.com") && pathname.endsWith(".git")) {
    return "git-fetch";
  }
  ...
  hooks: {
    beforeRequest: [
      (options) => {
        ...
        if (process.env.CDXGEN_ALLOWED_HOSTS) {
          const hostAllowed = !!isAllowedHost(options.url.hostname);
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (resolvedFile !== file) {
            recordObservedActivity("path-resolution", join(basePath, file), {
              metadata: {
                capability: "realpath-resolution",
