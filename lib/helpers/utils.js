  ...
  for (const p of pkgList) {
    try {
      ...
      if (!p._integrity && versionToUse.checksum) {
        p._integrity = `sha256-${Buffer.from(versionToUse.checksum, "hex").toString("base64")}`;
      }
      if (!p.properties) {
        p.properties = [];
      }
...
function resolveCargoWorkspaceMembers(workspaceRootFile, workspaceData) {
  ...
  for (const memberPattern of workspaceData?.members || []) {
    ...
    const matchedMemberFiles = globSync(
      resolve(workspaceRootDir, memberPattern, "Cargo.toml"),
      {
        nodir: true,
      },
    );
...
export async function parseCargoTomlData(
  ...
  if (visitedCargoTomlFiles.has(cargoTomlFile)) {
    return pkgList;
  }
  visitedCargoTomlFiles.add(cargoTomlFile);
  const cargoData = readCargoTomlData(cargoTomlFile);
  if (!cargoData) {
    return pkgList;
  }
  const workspaceContext = resolveCargoWorkspaceContext(
  ...
  const addPackageToList = (packageList, newPackage, { simple }) => {
    ...
    if (newPackage._integrity) {
      component.hashes = [
        {
          alg: "SHA-256",
          content: newPackage._integrity.replace(/^sha256-/i, ""),
        },
      ];
    }
