 ...
 */
function buildColliderComponent(
  name,
  pkgData = {},
  lockFile = undefined,
  ...
  if (typeof pkgData?.origin === "string" && pkgData.origin.trim()) {
    try {
      ...
      thoughtLog("Ignoring invalid Collider origin URL", pkgData.origin);
    }
  }
  const component = {
    name,
 ...
 * @returns {{ pkgList: Object[], dependencies: Object, parentComponentDependencies: string[] }}
 */
export function parseColliderLockData(colliderLockData, lockFile = undefined) {
  const pkgList = [];
  const dependencies = {};
  const parentComponentDependencies = [];
  if (!colliderLockData) {
