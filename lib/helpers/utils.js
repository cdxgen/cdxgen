...
function normalizeRecordedPathForComparison(
  ...
  if (basePath && path.isAbsolute(candidatePath)) {
    ...
    if (
      ...
      normalizedPath = relative(resolvedBasePath, resolvedCandidatePath);
    }
  }
...
function collectDirectoryExtractionStats(targetPath) {
...
export async function safeExtractArchive(
  ...
  const sourceBytes = traceArchiveStats
    ...
    : undefined;
  const beforeStats = traceArchiveStats
    ? collectDirectoryExtractionStats(targetPath)
    : undefined;
  if (isDryRun) {
    recordActivity({
      archiveKind: kind,
  ...
  try {
    await extractor();
    const afterStats = traceArchiveStats
      ? collectDirectoryExtractionStats(targetPath)
      : undefined;
    recordActivity({
      archiveKind: kind,
      ...
      ...(beforeStats && afterStats
        ? {
            extractedBytes: Math.max(
              0,
              afterStats.totalBytes - beforeStats.totalBytes,
            ),
            extractedFileCount: Math.max(
              0,
              afterStats.fileCount - beforeStats.fileCount,
            ),
          }
        : {}),
      status: "completed",
  ...
  hooks: {
    beforeRequest: [
      (options) => {
        ...
        if (allowedHostsEnv) {
          const hostAllowed = !!isAllowedHttpHost(options.url.hostname);
        ...
        if (!isAllowedHttpHost(options.url.hostname)) {
          console.log(
