  ...
  for (const apath of binPaths) {
    try {
      ...
      for (const file of files) {
        ...
        // Prefer canonical usr-merged aliases such as usr/bin over bin when both
        // resolve to the same target, then fall back to lexicographic ordering.
        if (
          !existingFile ||
          (file.startsWith("usr/") && !existingFile.startsWith("usr/")) ||
          (file.startsWith("usr/") === existingFile.startsWith("usr/") &&
            file < existingFile)
        ) {
