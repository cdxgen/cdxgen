...
function collectPythonLockDistributionReferences(pkg) {
  ...
  const vcsUrl = pkg?.vcs?.url || pkg?.vcs?.git || pkg?.source?.git;
  if (typeof vcsUrl === "string" && vcsUrl.trim()) {
    addDistributionReference(
      vcsUrl.startsWith("git+") ? vcsUrl : `git+${vcsUrl}`,
      "vcs",
    );
  }
  if (pkg?.source?.url) {
...
export async function parsePyLockData(lockData, lockFile, pyProjectFile) {
  ...
  for (const apkg of packageEntries) {
    ...
    const distributionReferences =
      collectPythonLockDistributionReferences(apkg);
    if (distributionReferences.length) {
      pkg.externalReferences = distributionReferences;
    }
    if (pyLockMode) {
