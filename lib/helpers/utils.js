...
const ATOM_PYTHON_DEFAULT_IGNORE_DIRS = [
  "samples",
  "docs",
  "virtualenvs",
  "venv",
    ...
    if (char === "*" && nextChar === "*" && nextNextChar === "/") {
      ...
      if (braceEnd === -1) {
        ...
        const alternates = pattern
          .slice(i + 1, braceEnd)
          .split(",")
          .map((alternate) => escapeScalaRegexLiteral(alternate));
 ...
 *
 * Atom language frontends consume ignore values as Scala regexes, while cdxgen
 * exposes `--exclude` as glob patterns. The returned regex is intentionally
 * path-separator agnostic so the same value works on POSIX and Windows paths.
 *
...
export function globPatternsToAtomIgnoreRegex(patterns = []) {
  const fragments = patterns
    ...
    .filter(Boolean)
    .filter((pattern) => !pattern.startsWith("!"))
    .map((pattern) => globToScalaRegexFragment(pattern));
  if (!fragments.length) {
    return undefined;
  }
...
function extractIgnoreDirsFromExcludePatterns(patterns) {
  ...
  for (const pattern of patterns) {
    ...
    if (dirName && (!dirName.includes(".") || segments.at(-1) !== dirName)) {
      ignoreDirs.add(dirName.toLowerCase());
    }
  }
...
export function buildAtomCommandEnv(options = {}, language = "") {
  ...
  if (normalizedLanguage === "python") {
    const pythonBaseIgnoreDirs =
      process.env.CHEN_PYTHON_IGNORE_DIRS ?? ATOM_PYTHON_DEFAULT_IGNORE_DIRS;
    env.CHEN_PYTHON_IGNORE_DIRS = mergeCsvValues(
      pythonBaseIgnoreDirs,
      atomIgnoreRegex,
    );
  }
  if (ATOM_JS_LANGUAGES.has(normalizedLanguage)) {
    ...
    env.ASTGEN_IGNORE_DIRS = mergeCsvValues(
