...
const NPM_LIFECYCLE_JS_RUNNER_PATTERN =
  /\b(?:node|tsx|ts-node|babel-node)\b(?:\s+--?[A-Za-z0-9-]+(?:[=\s][^|\s;&]+)*)*\s+(['"]?)([^'"|;&]+?\.[cm]?[jt]sx?)\1/g;
...
function extractLifecycleScriptSourceFiles(pkgJsonFile, scriptValue) {
  ...
  for (const match of scriptValue.matchAll(NPM_LIFECYCLE_JS_RUNNER_PATTERN)) {
    const relativeFile = match?.[2]?.trim();
    if (!relativeFile) {
      continue;
    }
    const resolvedFile = resolve(dirname(pkgJsonFile), relativeFile);
    if (!safeExistsSync(resolvedFile) || seen.has(resolvedFile)) {
      continue;
    }
...
const hasValidPercentEncoding = (value) => {
  for (let index = 0; index < value.length; index++) {
    ...
    const trailingCode = value.charCodeAt(index + 3);
    if (Number.isFinite(trailingCode) && isAsciiHexCode(trailingCode)) {
      return false;
    }
    index += 2;
  }
