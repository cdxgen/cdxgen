...
function extractIgnoreDirsFromExcludePatterns(patterns) {
  ...
  for (const pattern of patterns) {
    ...
    if (
      dirName &&
      (isExactPath || !dirName.includes(".") || segments.at(-1) !== dirName)
    ) {
 ...
 */
export function buildAtomCommandEnv(options = {}) {
  const excludePatterns = getExcludePatterns(options);
  if (!excludePatterns.length) {
    return {};
  }
  const ignoreDirs = mergeCsvValues(
  ...
  return { CHEN_IGNORE_DIRS: ignoreDirs };
}
