 ...
 */
function buildColliderComponent(name, version = "") {
  if (!name) {
    return undefined;
  }
  const purl = new PackageURL(
 ...
 * @returns {{ pkgList: Object[], dependencies: Object, parentComponentDependencies: string[] }}
 */
export function parseColliderLockData(colliderLockData) {
  const pkgList = [];
  const dependencies = {};
  const parentComponentDependencies = [];
  if (!colliderLockData) {
  ...
  let lockFile;
  try {
    lockFile = JSON.parse(colliderLockData);
  } catch {
    return { pkgList, dependencies, parentComponentDependencies };
  }
  const addedBomRefs = new Set();
  const directDependencies = lockFile.dependencies || {};
  const packages = lockFile.packages || {};
  for (const [name, pkgData] of Object.entries(directDependencies)) {
    const component = buildColliderComponent(name, pkgData?.version || "");
    if (!component) {
      continue;
    }
