...
export function trimJarGroupSuffix(group, name) {
  ...
  // Only trim when the artifact name is compound.
  if (!name.includes("-") && !name.includes(".")) {
    return group;
  }
  const dottedName = name.toLowerCase().replace(/-/g, ".");
  if (group.includes(`.${dottedName}`)) {
    return group.replace(new RegExp(`.${dottedName}$`), "");
  }
  const lowerName = name.toLowerCase();
  if (group.includes(`.${lowerName}`)) {
    return group.replace(new RegExp(`.${lowerName}$`), "");
  }
  return group;
}
