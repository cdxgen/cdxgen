 ...
 *
 * @param {Object} lockData TOML data from poetry.lock, pdm.lock, uv.lock, or pylock.toml file
...
export async function parsePyLockData(lockData, lockFile, pyProjectFile) {
  ...
  for (const apkg of packageEntries) {
    ...
    if (apkg["python-versions"] || apkg["requires-python"]) {
      pkg.properties.push({
        name: "cdx:pypi:requiresPython",
        value: apkg["python-versions"] || apkg["requires-python"],
      });
    }
    ...
    if (pyLockMode) {
      pkg.properties = pkg.properties.concat(collectPyLockPackageProperties(apkg));
    }
    ...
    if (
      ...
      if (Array.isArray(apkg.dependencies)) {
        ...
        for (const apkgDep of allDeps) {
          ...
          depsMap[pkg["bom-ref"]].add(
            existingPkgMap[nameLower] || existingPkgMap[nameStr] || nameStr,
          );
          // Propagate the workspace properties to the child components
          if (
            (existingPkgMap[nameLower] || existingPkgMap[nameStr]) &&
            pkgBomRefMap[existingPkgMap[nameLower] || existingPkgMap[nameStr]]
          ) {
            const dependentPkg =
              pkgBomRefMap[existingPkgMap[nameLower] || existingPkgMap[nameStr]];
