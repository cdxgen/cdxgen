...
export function recordSymlinkResolution(
  ...
  if (!sourcePath || !resolvedPath || sourcePath === resolvedPath) {
    return undefined;
  }
  return recordObservedActivity("symlink-resolution", sourcePath, {
    metadata: {
    ...
    reason:
      options.reason || `Resolved symlink ${sourcePath} to ${resolvedPath}.`,
    status: options.status || "completed",
    traceDetail: resolvedPath,
  });
}
