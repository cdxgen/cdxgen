...
function dotnetLocationFromDosai(item) {
...
function dotnetSourceLocationFromDosaiNode(node) {
  const location = dotnetLocationFromDosai(node);
...
function dotnetSourceLocationFromDosaiLocation(location) {
  const sourceLocation = dotnetLocationFromDosai(location);
...
function addDotnetSetValue(map, key, value) {
  if (!key || !value) {
    return;
  }
  map[key] ??= new Set();
...
function addDotnetIdentityMethod(apkg, value) {
  ...
  const identity = Array.isArray(apkg.evidence.identity)
    ? (apkg.evidence.identity[0] ?? {
        field: "purl",
        confidence: 1,
        methods: [],
      })
    : (apkg.evidence.identity ?? { field: "purl", confidence: 1, methods: [] });
  identity.field ??= "purl";
  ...
  apkg.evidence.identity = Array.isArray(apkg.evidence.identity)
    ? [identity]
    : identity;
}
...
export function addEvidenceForDotnet(pkgList, slicesFile) {
  ...
  for (const apkg of pkgList) {
    if (apkg.purl) {
      purlAliasMap[apkg.purl] = apkg.purl;
      purlAliasMap[dotnetPurlKey(apkg.purl)] = apkg.purl;
    }
  ...
  if (slicesData && Object.keys(slicesData)) {
    ...
    if (slicesData.Dependencies) {
      for (const adep of slicesData.Dependencies) {
        if (adep.Purl) {
          const modPurl =
            purlAliasMap[adep.Purl] || purlAliasMap[dotnetPurlKey(adep.Purl)];
          if (modPurl) {
            addDotnetSetValue(
              purlLocationMap,
              modPurl,
              dotnetSourceLocationFromDosaiLocation(adep),
            );
            addDotnetSetValue(
              purlModulesMap,
              modPurl,
    ...
    if (slicesData.PackageReachability) {
      ...
      for (const reachability of slicesData.PackageReachability) {
        const modPurl =
          purlAliasMap[reachability.Purl] ||
          purlAliasMap[dotnetPurlKey(reachability.Purl)];
        if (!modPurl) {
          continue;
        }
        ...
        for (const sourceLocation of reachability.SourceLocations || []) {
          const location =
            dotnetSourceLocationFromDosaiLocation(sourceLocation);
          addDotnetSetValue(purlLocationMap, modPurl, location);
        ...
        for (const nodeId of reachability.NodeIds || []) {
          ...
          if (!hasExplicitSourceLocations) {
            addDotnetSetValue(
              purlLocationMap,
              modPurl,
              dotnetSourceLocationFromDosaiNode(node),
            );
          }
          addDotnetSetValue(
            purlModulesMap,
