...
/**
 * Safe wrapper around spawnSync for commands that must never execute through a shell.
 * This mirrors safeSpawnSync permission, dry-run, tracing, and activity recording
 * behavior while forcing `shell: false` before invoking child_process.spawnSync.
 *
 * @param {string} command The executable to spawn
 * @param {string[]} args Arguments to pass to the command
 * @param {Object} options Options forwarded to spawnSync (e.g. cwd, env)
 * @returns {Object} spawnSync result object with status, stdout, stderr, and error fields
 */
export function safeSpawnSyncNoShell(command, args, options) {
  const activityDescriptor = buildCommandActivityDescriptor(
    command,
    args,
    options,
      ...
      for (const reachability of slicesData.PackageReachability) {
        ...
        for (const edgeId of reachability.EdgeIds || []) {
          ...
          if (!hasExplicitSourceLocations) {
            addDosaiSetValue(purlLocationMap, modPurl, dosaiLocation(edge));
          }
          addDosaiSetValue(
            purlMethodsMap,
            modPurl,
