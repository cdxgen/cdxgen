...
const buildReadCountSuffix = (count) =>
  count > 1 ? ` (${count} times).` : ".";
...
export function recordEnvironmentRead(varName, options = {}) {
  if (!(isDryRun || dryRunReadTraceState.debugMode) || !varName) {
    return undefined;
  }
...
export function recordSensitiveFileRead(filePath, options = {}) {
  if (!(isDryRun || dryRunReadTraceState.debugMode) || !filePath) {
    return undefined;
  }
  const kind = options.kind || "read";
  const label = options.label || "sensitive file";
  const status = options.status || "completed";
...
function installEnvironmentReadTracer() {
  if (!isNode) {
    return;
  }
  if (!dryRunReadTraceState.envProxy) {
    dryRunReadTraceState.rawEnv = process.env;
    dryRunReadTraceState.envProxy = new Proxy(dryRunReadTraceState.rawEnv, {
      deleteProperty(target, prop) {
        return Reflect.deleteProperty(target, prop);
      },
      get(target, prop, receiver) {
        if (typeof prop === "string") {
          recordEnvironmentRead(prop);
        }
  ...
  if (process.env !== dryRunReadTraceState.envProxy) {
    process.env = dryRunReadTraceState.envProxy;
  }
}
...
export function recordActivity(activity) {
  if (!(isDryRun || dryRunReadTraceState.debugMode)) {
    return undefined;
  }
...
dryRunReadTraceState.debugMode = DEBUG_MODE;
