    ...
    if (node === rootNode) {
      ...
      if (npmManifestSources.length) {
        addComponentProperty(
          ...
          npmManifestSources.map((manifestSource) => manifestSource.type).join(","),
        );
        addComponentProperty(
          pkg,
...
function applyManifestSourceProperties(component, propertyPrefix, manifestSource) {
...
function parsePyProjectDependencySourceString(value) {
  ...
  const directReferenceMatch = value.match(
    /^\s*([A-Za-z0-9_.-]+)(?:\[[^\]]+\])?\s*@\s*(\S+)/,
  );
...
async function parseReqData(reqFile, reqData = null, fetchDepsInfo = false) {
  ...
  for (const line of lines) {
    ...
    if (requirementManifestSource?.name) {
      ...
      applyManifestSourceProperties(apkg, "cdx:pypi", requirementManifestSource);
