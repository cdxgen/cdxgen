...
function _parseYarnLine(l) {
  ...
  let group = "";
  let fullName;
  // Aliased dependencies encode the real package name in the npm: target.
  // Eg: react-is-18@npm:react-is@^18.3.1 (alias) vs lru-cache@npm:^6.0.0 (plain).
  // The component should be named after the real package, not the alias.
  const npmIdx = l.indexOf("@npm:");
  if (npmIdx > -1) {
    const target = l.slice(npmIdx + "@npm:".length);
    // For an alias the target is `name@range`; for a plain berry entry it is
    // just a `range`. A leading "@" only marks a scoped package, so look for an
    // "@" separator beyond it to distinguish the two.
    const sepIdx = target.startsWith("@")
      ? target.indexOf("@", 1)
      : target.indexOf("@");
    if (sepIdx > -1) {
      // Aliased dependency: use the real package name from the target.
      fullName = target.slice(0, sepIdx);
    } else {
      // Plain entry: the name is the text before "@npm:".
      fullName = l.slice(0, npmIdx);
    }
  } else {
    // Yarn v1 plain entry: name@range or @scope/name@range.
    ...
    if (tmpA.length >= 2) {
      fullName = (prefixAtSymbol ? "@" : "") + tmpA[0];
    }
  }
  if (fullName) {
