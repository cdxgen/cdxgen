...
function collectPythonLockDistributionReferences(pkg) {
  ...
  function addDistributionReference(url, comment) {
  ...
  addDistributionReference(pkg?.archive?.url, "archive");
  addDistributionReference(pkg?.sdist?.url, "sdist");
  if (Array.isArray(pkg?.wheels)) {
    for (const wheel of pkg.wheels) {
      addDistributionReference(wheel?.url, "wheel");
    }
  }
  const vcsSource = [
  ...
  if (vcsSource) {
    ...
    const normalizedVcsUrl =
      vcsSource.kind === "git" && !vcsUrl.startsWith("git+")
        ? `git+${vcsUrl}`
        : vcsUrl;
    addDistributionReference(normalizedVcsUrl, "vcs");
  }
  if (pkg?.source?.url) {
    addDistributionReference(pkg.source.url, "source");
  }
...
function collectPythonLockMetadataFileEntries(lockTomlObj, pkg) {
  ...
  const expectedKeys = new Set([normalizeKey(pkg.name)]);
  if (pkg.version) {
    expectedKeys.add(
      `${normalizeKey(pkg.name)} ${`${pkg.version}`.trim().toLowerCase()}`,
    );
  }
  ...
  for (const [entryKey, entryValues] of Object.entries(
    ...
    if (expectedKeys.has(normalizeKey(entryKey))) {
...
function mergeExternalReferences(component, references) {
  ...
  const seen = new Set(
    existingReferences.map(
      (reference) =>
        `${reference.type}|${reference.url}|${reference.comment || ""}`,
    ),
  );
  for (const reference of references) {
    const dedupeKey = `${reference.type}|${reference.url}|${reference.comment || ""}`;
