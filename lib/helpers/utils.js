...
function normalizeRecordedPathForComparison(
  ...
  if (basePath && path.isAbsolute(candidatePath)) {
    const resolvedBasePath = resolve(basePath).replaceAll("\\", "/");
    const resolvedCandidatePath = resolve(candidatePath).replaceAll("\\", "/");
    if (
      resolvedCandidatePath === resolvedBasePath ||
      resolvedCandidatePath.startsWith(`${resolvedBasePath}/`)
    ) {
      normalizedPath = relative(
        resolvedBasePath.replaceAll("/", path.sep),
        resolvedCandidatePath.replaceAll("/", path.sep),
      ).replaceAll("\\", "/");
    }
  }
