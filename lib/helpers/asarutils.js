...
function createAsarSigningComponent(archivePath, signingInfo, options = {}) {
  const componentType =
    options.specVersion && options.specVersion < 1.6
      ? "data"
      : "cryptographic-asset";
 ...
 * @param {Object} [_options={}] Parse options
 * @param {string} [_options.asarVirtualPath] Virtual archive identity to use in
 * BOM references and evidence for nested ASAR recursion
 * @param {number} [_options.specVersion] CycloneDX spec version used to choose
...
export async function parseAsarArchive(archivePath, _options = {}) {
  ...
  const archiveIdentityPath =
    typeof _options?.asarVirtualPath === "string" && _options.asarVirtualPath
      ? _options.asarVirtualPath
      : resolvedArchivePath;
