...
function inferMainEntryFlags(packageJson) {
  ...
  addSanitizedProperty(properties, "cdx:asar:productName", packageJson?.productName);
  const lifecycleScripts = Object.keys(packageJson?.scripts || {}).filter((name) =>
    ASAR_LIFECYCLE_SCRIPT_NAMES.has(name),
  );
...
export function rewriteExtractedArchivePaths(subject, extractedDir, archivePath) {
...
function collectArchiveSummaryProperties(
  ...
  addSanitizedProperty(properties, "cdx:asar:entryCount", `${summary.entryCount}`);
  addSanitizedProperty(properties, "cdx:asar:fileCount", `${summary.fileCount}`);
  addSanitizedProperty(properties, "cdx:asar:directoryCount", `${summary.directoryCount}`);
  addSanitizedProperty(properties, "cdx:asar:symlinkCount", `${summary.symlinkCount}`);
  addSanitizedProperty(properties, "cdx:asar:jsFileCount", `${summary.jsFileCount}`);
  addSanitizedProperty(
    properties,
...
function createArchiveParentComponent(
  ...
  const archivePurl =
    createAsarPackagePurl(primaryPackageJson?.name, primaryPackageJson?.version) ||
...
function createArchiveEntryComponent(archivePath, entry, computedHash, jsAnalysis, suspiciousAnalysis) {
  ...
  addSanitizedProperty(properties, "cdx:asar:unpacked", String(entry.unpacked === true));
...
export async function parseAsarArchive(archivePath, _options = {}) {
  ...
  for (const entry of parsedArchive.entries) {
    ...
    if (ASAR_JS_ANALYSIS_EXTENSIONS.has(extname(entry.path))) {
      ...
      let tempSourceFile;
      try {
        tempSourceFile = safeMkdtempSync(join(getTmpDir(), "asar-js-"));
        const sourcePath = join(tempSourceFile, basename(entry.path));
        if (fileBuffer) {
          safeWriteSync(sourcePath, fileBuffer);
        ...
        if (safeExistsSync(sourcePath)) {
          jsAnalysis = analyzeJsCapabilitiesFile(sourcePath);
          suspiciousAnalysis = analyzeSuspiciousJsFile(sourcePath);
        }
      } finally {
        if (tempSourceFile) {
          safeRmSync(tempSourceFile, { force: true, recursive: true });
        }
      }
    }
    jsAnalysis?.capabilities?.forEach((capability) => {
...
function extractAsarArchive(archivePath, targetDir) {
  ...
  for (const entry of parsedArchive.entries) {
    ...
    if (!isPathWithin(targetDir, destinationPath)) {
      throw new Error(`Refusing to extract ASAR entry outside target dir: ${entry.path}`);
    }
    if (entry.type === "directory") {
    ...
    if (entry.unpacked) {
      safeCopyFileSync(resolveUnpackedEntryPath(archivePath, entry.path), destinationPath);
      continue;
    }
    ...
    if (entry.executable) {
      const extractedStat = lstatSync(destinationPath);
      safeWriteSync(destinationPath, fileBuffer, { mode: extractedStat.mode | 0o111 });
    }
  }
}
...
export async function extractAsarToTempDir(archivePath) {
  ...
  try {
    ...
    if (DEBUG_MODE) {
      console.log(`Error extracting ASAR archive ${archivePath}:`, error.message);
    }
...
export function buildAsarExtractionSummary(archiveAnalysis, extractionPerformed) {
  const properties = [];
  if (archiveAnalysis?.packageManifestPaths?.length) {
    addSanitizedProperty(
      ...
      archiveAnalysis.packageManifestPaths
        .map((manifestPath) => toArchiveOccurrence("", manifestPath).replace(/^#/, ""))
        .join(", "),
    );
  }
