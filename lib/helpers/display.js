...
export function printActivitySummary(reportType = undefined) {
  ...
  const createActivitySummaryPayload = () => ({
    activities,
    mode: isDryRun ? "dry-run" : "debug",
    summary: {
  ...
  const activitySummaryPayload = createActivitySummaryPayload();
  if (reportType === "json") {
    console.log(JSON.stringify(activitySummaryPayload, null, 2));
    return;
  }
  if (reportType === "jsonl") {
    console.log(
