...
function mergeAlgorithmComponentUsage(component, usage, src) {
  ...
  if (typeof usage.lineNumber === "number") {
    const locationValue = `${usage.fileName || "<inline>"}:${usage.lineNumber}${typeof usage.columnNumber === "number" ? `:${usage.columnNumber}` : ""}`;
    if (
      !properties.some(
        (property) =>
...
export async function collectSourceCryptoComponents(src, options = {}) {
  ...
  for (const usage of inventory.algorithms || []) {
    ...
    mergeAlgorithmComponentUsage(component, usage, src);
    componentsByRef.set(bomRef, component);
  }
  return Array.from(componentsByRef.values()).sort((left, right) =>
