...
export const assertProtoSupportedSpecVersion = (
  ...
  if (
    ...
    isProtoSupportedSpecVersion(normalizedSpecVersion)
  ) {
    return;
  }
  throw new Error(
    `CycloneDX ${normalizedSpecVersion} is not currently supported for ${operation}. @appthreat/cdx-proto supports ${supportedSpecVersions.join(", ")} only.`,
  );
};
