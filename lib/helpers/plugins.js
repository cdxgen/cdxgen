...
/**
 * Resolve the cdxgen companion plugins directory when running from an npx
 * cache layout.
 *
 * In npx (Deno, npm, etc.) the cdxgen package lives alongside
 * @cdxgen/cdxgen-plugins-bin under a shared registry root such as
 * `~/.cache/deno/npm/registry.npmjs.org/`.  This helper walks up from
 * `dirNameStr` to find that root, then constructs the plugins path.
 *
 ...
 */
function resolveNpxPluginsDir(dirNameStr, pluginVersion, target) {
  ...
  const idx = parts.lastIndexOf("registry.npmjs.org");
  if (idx === -1) {
    return undefined;
  }
  const registryRoot = parts.slice(0, idx).join("/");
  return join(
    registryRoot,
    "@cdxgen",
...
export function resolveCdxgenPlugins() {
  ...
  if (!pluginsDir) {
    ...
    if (!pluginsDir) {
      const npxPluginsDir = resolveNpxPluginsDir(
        dirNameStr,
        pluginVersion,
        target,
      );
      if (npxPluginsDir && hasUsablePluginsDir(npxPluginsDir)) {
        pluginsDir = npxPluginsDir;
        if (DEBUG_MODE) {
