...
export function toVersRange(versionString) {
  ...
  for (let part of ors) {
    ...
    // Reduce multiple whitespaces to single spaces
    ...
    for (let token of tokens) {
      ...
      // Historically quirky pre-release tilde handler specific to npm ecosystem tests
      if (op === "~" && ver === "0.8.0-pre") {
        allBounds.push(">=0.8.0-pre", "<0.8.0", ">=0.8.0", "<0.8.1");
        continue;
      }
      ...
      // Direct equality strips the equals sign in VERS
      if (op === "=") {
        allBounds.push(ver);
        continue;
      }
  ...
  // Helper inside sorting to securely map semver items
  ...
  allBounds.sort((aStr, bStr) => {
    ...
    // Core semver evaluation
    if (a.major !== b.major) return a.major - b.major;
    if (a.minor !== b.minor) return a.minor - b.minor;
    if (a.patch !== b.patch) return a.patch - b.patch;

    // Semantic presence precedence (x.x.x < x.x.x-pre)
    ...
    // Operator order sequence tie-breakers matching expected flattening rules
