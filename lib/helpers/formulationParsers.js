...
/**
 * Deduplicate a components array by purl / bom-ref / name+version key,
 * merging properties when the same component appears more than once.
 *
 * @param {Object[]} components
 * @returns {Object[]}
 */
function trimComponents(components) {
  const keyCache = {};
  const filtered = [];
  for (const comp of components || []) {
    ...
    if (!keyCache[key]) {
      keyCache[key] = comp;
      filtered.push(comp);
    } else if (comp.properties) {
      if (!keyCache[key].properties) {
        keyCache[key].properties = [];
      }
      keyCache[key].properties = keyCache[key].properties.concat(
        comp.properties,
      );
    }
  }
 ...
 *   patterns: string[],                       // glob patterns for file discovery
 *   parse(files: string[], options: Object):  // sync or async (Promise-returning) function
...
export function registerParser(parser) {
  if (!parser?.id || !parser?.patterns || typeof parser?.parse !== "function") {
    throw new TypeError(
      "registerParser: parser must have id (string), patterns (string[]), and parse (function)",
    );
  }
 ...
 */
export function addFormulationSection(options, context = {}) {
  ...
  if (options.specVersion >= 1.6 && Object.keys(treeHashes).length === 2) {
    // Bug fix: use correct `gitoid:commit:sha1:` type tag for commit and tree
    // hashes (not `gitoid:blob:sha1:` which is reserved for file-content blobs).
    parentOmniborId = `gitoid:commit:sha1:${treeHashes.parent}`;
    treeOmniborId = `gitoid:commit:sha1:${treeHashes.tree}`;
    components.push({
      type: "file",
      name: "git-parent",
  ...
  for (const parser of _parsers) {
    ...
    let result;
    try {
      result = parser.parse(matchedFiles, options);
    } catch (err) {
