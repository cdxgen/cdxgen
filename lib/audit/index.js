...
export async function runAuditFromBoms(inputBoms, options) {
  ...
  if (isDryRun) {
    ...
    const report = {
      ...
      results: extractedTargets.targets.map((target) => ({
        assessment: {
          ...
          findingsCount: 0,
          reasons: [
            "Dry run mode planned this predictive audit target but skipped registry metadata fetches, upstream repository cloning, and child SBOM generation.",
          ],
          score: 0,
          severity: "none",
        },
    ...
    recordActivity({
      kind: "audit",
      reason:
        "Dry run mode planned predictive BOM audit targets but skipped registry metadata fetches, repository cloning, and child SBOM generation.",
