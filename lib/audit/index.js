...
async function recheckFindingsAgainstDefaultBranch(
  ...
  try {
    ...
    cloneRepositoryToDir(resolution.repoUrl, defaultBranchDir, undefined);
    ...
    for (const finding of defaultBranchFindings) {
      defaultBranchFindingKeys.add(finding?.ruleId);
      defaultBranchFindingKeys.add(
        `${finding?.ruleId}|${finding?.location?.file}`,
      );
    }
    ...
    for (const finding of findings) {
      ...
      const fileKey = `${finding?.ruleId}|${finding?.location?.file}`;
      if (
        defaultBranchFindingKeys.has(fileKey) ||
        defaultBranchFindingKeys.has(finding?.ruleId)
      ) {
        retainedFindings.push(finding);
      } else {
        fixedCount += 1;
