...
const PYTHON_EXECUTION_PATTERN =
  /\b(?:exec\s*\(|eval\s*\(|compile\s*\(|subprocess\.(?:Popen|run|call|check_output)|os\.(?:system|popen))\b/i;
...
function inspectPythonHeuristicFile(filePath) {
  ...
  if (
    Buffer.byteLength(fileContent, "utf8") > PYTHON_HEURISTIC_MAX_FILE_BYTES
  ) {
    return undefined;
  }
  const indicators = [];
