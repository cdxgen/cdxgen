...
function buildPredictiveAuditPreflightMessage(extractedTargets, options) {
  ...
  const allowlistExclusionMessage = allowlistedTargetsExcluded
    ? ` Skipping ${allowlistedTargetsExcluded} allowlisted package(s) from the well-known purl prefix filter${options?.allowlistFile ? " and your custom allowlist" : ""}.`
    : "";
...
export async function runAuditFromBoms(inputBoms, options) {
  ...
  if (options.allowlistFile) {
    targetSelectionOptions.allowlistFile = options.allowlistFile;
  }
  const extractedTargets = collectAuditTargets(inputBoms, targetSelectionOptions);
  if (isDryRun) {
