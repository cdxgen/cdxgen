...
export function buildTargetContextFindings(target) {
  ...
  const hasTrustedPublishing =
    getTargetProperty(target, "cdx:npm:trustedPublishing") === "true" ||
    getTargetProperty(target, "cdx:pypi:trustedPublishing") === "true";
  const hasProvenanceUrl =
    Boolean(getTargetProperty(target, "cdx:npm:provenanceUrl")) ||
    Boolean(getTargetProperty(target, "cdx:pypi:provenanceUrl"));
  if (target.type === "npm") {
    ...
    if (
      ...
      findings.push({
        ...
        message: `npm package '${target.name}@${target.version}' has install-time execution hooks but no registry-visible trusted publishing or provenance URL.`,
        mitigation:
          "Prefer versions with registry-visible provenance, review install scripts carefully, and pin/allowlist publishers for high-risk packages.",
        ruleId: "PROV-001",
        severity: "medium",
      });
    }
...
export async function auditTarget(target, options) {
  ...
  try {
    ...
    const predictiveFindings = findings.concat(contextualFindings);
    ...
    if (options.reportsDir) {
      ...
      writeJsonFile(result.findingsFile, predictiveFindings);
      writeJsonFile(result.summaryFile, {
        assessment,
        findingsCount: predictiveFindings.length,
        repoUrl: sanitizedRepoUrl,
        sourceDirectoryConfidence: sourceSelection.confidence,
