...
function buildPredictiveAuditPreflightMessage(extractedTargets, options) {
  ...
  const allowlistExclusionMessage = allowlistedTargetsExcluded
    ? ` Skipping ${allowlistedTargetsExcluded} allowlisted package(s) using the built-in well-known purl prefix filter${options?.allowlistFile ? " and your custom allowlist" : ""}.`
    : "";
