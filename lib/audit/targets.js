...
import { getNpmMetadata, getPyMetadata } from "../helpers/utils.js";
...
export async function enrichInputBomsWithRegistryMetadata(inputBoms) {
  ...
  for (const inputBom of inputBoms || []) {
    ...
    for (const component of components) {
      ...
      const candidate = buildRegistryMetadataCandidate(componentPurl, component);
      if (!candidate) {
        continue;
      }
  ...
  if (!npmCandidates.length && !pypiCandidates.length) {
    return;
  }
  ...
  try {
    const enrichedCandidates = [
      ...
      ...(pypiCandidates.length ? await getPyMetadata(pypiCandidates, false) : []),
    ];
    for (const candidate of enrichedCandidates) {
      const matchedComponents = componentRefs.get(candidate._auditMetadataKey) || [];
      const registryProperties = (candidate.properties || []).filter((property) =>
        registryMetadataPropertyName(property?.name),
      );
