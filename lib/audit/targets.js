...
function getComponentPropertyValue(component, propertyName) {
  return component?.properties?.find((property) => property.name === propertyName)
    ?.value;
}
...
function isPlatformSpecificComponent(component, purlType) {
  if (purlType === "npm") {
  ...
  if (purlType === "pypi") {
    ...
    ].some((propertyName) => hasNonEmptyComponentProperty(component, propertyName));
  }
  return false;
}

function isDevelopmentOnlyComponent(component, purlType) {
  if (purlType === "npm") {
...
export function collectAuditTargets(inputBoms, options) {
  ...
  return {
    ...
    stats: {
      ...
      developmentOnlyTargets: targets.filter((target) => target.developmentOnly)
        .length,
      nonRequiredTargets: nonRequiredTargets.length,
      platformSpecificTargets: targets.filter((target) => target.platformSpecific)
        .length,
