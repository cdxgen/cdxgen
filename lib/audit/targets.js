...
function normalizeAllowlistPrefixes(prefixes) {
  return [
    ...new Set(
      (prefixes || []).map(normalizeAllowlistPrefix).filter(Boolean),
    ),
  ];
}
...
function loadAllowlistPrefixes(filePath, sourceLabel) {
  try {
    return parseAllowlistPrefixes(readFileSync(filePath, "utf8"), sourceLabel);
  } catch (error) {
    throw new Error(
      `Failed to load ${sourceLabel} from ${filePath}: ${error.message}`,
    );
  }
}
...
function findMatchingAllowlistPrefix(componentPurl, allowlistPrefixes) {
  ...
  return allowlistPrefixes.find((prefix) => normalizedPurl.startsWith(prefix));
}
