...
describe("collectAuditTargets()", () => {
  ...
  it("supports additive custom predictive audit allowlists", () => {
    ...
    const allowlistFile = path.join(tmpDir, "custom-allowlist.json");
    writeFileSync(
      allowlistFile,
  ...
  it("supports newline-delimited custom allowlists with comments", () => {
    const tmpDir = mkdtempSync(path.join(os.tmpdir(), "cdx-audit-allowlist-"));
    const allowlistFile = path.join(tmpDir, "custom-allowlist.txt");
    writeFileSync(
      allowlistFile,
      [
      ...
      const inputBoms = [
        {
          ...
          source: "allowlist-custom-text.json",
        },
      ];
      ...
      const inputBoms = [
        {
          ...
          source: "allowlist-custom-prefixes.json",
        },
      ];
  ...
  it("requires a purl boundary after an allowlisted prefix", () => {
    ...
    assert.deepStrictEqual(
      collected.targets.map((target) => target.purl),
      [
        "pkg:npm/npm-check-updates@17.1.0",
        "pkg:npm/npm-run-all@4.1.5",
      ],
    );
