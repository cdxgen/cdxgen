 ...
 *
 * @param {string} path to the project path (unused for discovery, kept for signature consistency)
 * @param {Object} options Parse options from the cli
 * @returns {Promise<Object>} Promise resolving to BOM object
 */
export async function createChromeExtensionBom(path, options) {
  ...
  return buildBomNSData(options, pkgList, CHROME_EXTENSION_PURL_TYPE, {
    src: path,
    filename: chromeDirs.map((d) => d.dir).join(", "),
    nsMapping: {},
    dependencies: [],
  });
}
