...
function getIncludedAiInventoryTypes(options) {
  ...
  if (exactAiInventoryType) {
    return requestedAiInventoryTypes.filter(
  ...
  if (auditCategories.includes("ai-inventory")) {
    includedAiInventoryTypes.push(...AI_INVENTORY_PROJECT_TYPES);
  }
...
export async function createVscodeExtensionBom(path, options) {
  ...
  let dependencies = [];
  const tempDirs = [];
  const selectedProjectTypes = Array.isArray(options.projectType)
    ? options.projectType
    : [options.projectType];
  const shouldDiscoverInstalledIdeExtensions =
    selectedProjectTypes.includes("ide-extension") ||
    selectedProjectTypes.includes("ide-extensions");
