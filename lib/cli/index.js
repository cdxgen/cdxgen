...
function shouldDetectMcpInventory(options, allImports = {}) {
  ...
  if (auditCategories.some((category) => ["mcp-server", "ai-agent"].includes(category))) {
    return true;
  }
...
function summarizeAiInventoryNames(subjects, discoveryPath, kindSet) {
  return [...new Set(
    (subjects || [])
      .filter((subject) => kindSet.has(propertyValue(subject, "cdx:file:kind")))
      .map((subject) => propertyValue(subject, "SrcFile"))
      .filter(Boolean)
      .map((filePath) => relative(discoveryPath, filePath) || basename(filePath)),
  )].sort();
}

function summarizeAiInventoryServiceNames(services) {
  return [...new Set((services || []).map((service) => service?.name).filter(Boolean))].sort();
}

function formatAiInventorySummaryLine(label, count, names) {
  return `  ${label.padEnd(20)} ${count}${names.length ? ` (${names.join(", ")})` : ""}`;
}
...
function emitAiInventorySummary(aiInventory, discoveryPath) {
  ...
  const mcpServiceNames = summarizeAiInventoryServiceNames(aiInventory.services);
  console.warn(
    [
      ...
      formatAiInventorySummaryLine("Skill files:", summary.skillCount, skillNames),
      formatAiInventorySummaryLine(
        "MCP configs:",
        summary.mcpConfigCount,
        mcpConfigNames,
...
export async function createNodejsBom(path, options) {
  ...
  if (!exactAiInventoryType) {
    if (aiInventorySummary.instructionCount || aiInventorySummary.skillCount) {
      thoughtLog(
        `I found ${aiInventorySummary.instructionCount + aiInventorySummary.skillCount} AI skill/instruction file component(s). Use '--exclude-type ai-skill' for a package-only BOM, or '--bom-audit --bom-audit-categories ai-inventory --tlp-classification AMBER' when you want review-friendly reporting.`,
      );
    }
