  ...
  if (
    ...
    for (const f of pomFiles) {
      ...
      if (
        ...
        const tempMvnTree = join(basePath, "target", "cdxgen-mvn-tree.json");
        const tempMvnTreeText = join(basePath, "target", "cdxgen-mvn-tree.txt");
        ...
        if (f === firstPom) {
          ...
          if (result.status === 0) {
            const parentTreeFile = safeExistsSync(tempMvnParentTree)
              ? tempMvnParentTree
        ...
        if (
          result.status !== 0 ||
          result.error ||
          !safeExistsSync(tempMvnTree)
        ) {
          mvnTreeArgs = ["dependency:tree", `-DoutputFile=${tempMvnTreeText}`];
          if (addArgs.length) {
            mvnTreeArgs = mvnTreeArgs.concat(addArgs);
          }
          if (safeExistsSync(settingsXml)) {
        ...
        if (result.status !== 0 || result.error) {
          ...
          const treeFile = safeExistsSync(tempMvnTree)
            ? tempMvnTree
            : tempMvnTreeText;
          if (safeExistsSync(treeFile)) {
            ...
            if (tmpParentComponent && parsedList?.dependenciesList?.length) {
            ...
            if (!DEBUG_MODE) {
              ...
              if (safeExistsSync(tempMvnTreeText)) {
                safeUnlinkSync(tempMvnTreeText);
              }
            }
          }
        }
