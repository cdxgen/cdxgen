...
export async function createMultiXBom(pathList, options) {
  ...
  // Add formulation once at the top-level multi-BOM merge (not per-language)
  if (
    options.includeFormulation &&
    multiResult?.bomJson &&
    options.specVersion >= 1.5
    ...
    if (formulationData) {
      multiResult.bomJson.formulation = formulationData.formulation;
      if (formulationData.dependencies?.length) {
        multiResult.bomJson.dependencies = mergeDependencies(
          multiResult.bomJson.dependencies || [],
          formulationData.dependencies,
        );
      }
    }
