 ...
 * @returns {Promise<Object>} Promise resolving to BOM object
 */
function optionIncludesProjectType(optionValue, aliasKey) {
  const aliases = PROJECT_TYPE_ALIASES[aliasKey] || [aliasKey];
  const values = Array.isArray(optionValue)
    ? optionValue
    : optionValue
      ? [optionValue]
      : [];
  return values.some((value) => aliases.includes(String(value).toLowerCase()));
}

function getRequestedAiInventoryTypes(options) {
  return AI_INVENTORY_PROJECT_TYPES.filter((type) =>
    optionIncludesProjectType(options?.projectType, type),
  );
}

function getExcludedAiInventoryTypes(options) {
  return AI_INVENTORY_PROJECT_TYPES.filter((type) =>
    optionIncludesProjectType(options?.excludeType, type),
  );
}
...
export async function createNodejsBom(path, options) {
  ...
  const exactAiInventoryType =
    requestedAiInventoryTypes.length === 1 &&
    Array.isArray(options?.projectType) &&
    options.projectType.length === 1
  ...
  if (
    !hasAnyProjectType(["docker", "oci", "container", "os"], options, false) &&
    includedAiInventoryTypes.length
  ) {
  ...
  const aiSkillComponentCount = filterInventorySubjectsByTypes(
    aiInventory.components,
    ["ai-skill"],
  ).length;
  const mcpConfigComponentCount = filterInventorySubjectsByTypes(
    aiInventory.components,
    ["mcp"],
  ).filter((component) =>
    component.properties?.some(
      (property) =>
        property.name === "cdx:file:kind" && property.value === "mcp-config",
    ),
  ).length;
  if (!exactAiInventoryType) {
    if (aiSkillComponentCount) {
      thoughtLog(
        `I found ${aiSkillComponentCount} AI skill/instruction file component(s). Use '--exclude-type ai-skill' for a package-only BOM, or '--bom-audit --bom-audit-categories ai-agent --tlp-classification AMBER' when you want review-friendly reporting.`,
      );
    }
    if (mcpConfigComponentCount) {
      thoughtLog(
        `I found ${mcpConfigComponentCount} MCP config component(s). Use '--exclude-type mcp' to drop them, or '--bom-audit --bom-audit-categories mcp-server --tlp-classification AMBER' to keep and flag them.`,
      );
    }
  }
  if (exactAiInventoryType === "ai-skill") {
    ...
    let exactServices = [];
    exactComponents = trimComponents(
      exactComponents.concat(aiInventory.components),
    );
    exactDependencies = mergeDependencies(
      exactDependencies,
      aiInventory.dependencies,
    );
    exactServices = mergeServices(exactServices, aiInventory.services);
...
export async function createMultiXBom(pathList, options) {
  ...
  for (const path of pathList) {
    ...
    if (hasAnyProjectType(["oci", "js", "mcp", "ai-skill"], options)) {
      const exactAiInventoryType =
        getRequestedAiInventoryTypes(options).length === 1 &&
        Array.isArray(options?.projectType) &&
        options.projectType.length === 1
          ? getRequestedAiInventoryTypes(options)[0]
          : undefined;
      if (!hasAnyProjectType(["oci"], options, false)) {
        if (exactAiInventoryType === "mcp") {
          thoughtLog(
