...
import { createNpmWorkspacePurl } from "../helpers/npmutils.js";
 ...
 */
function isPkgTypeOnlyImport(allImports, group, name) {
  if (!allImports) {
    return false;
  }
  const pkgNames = [];
  ...
  if (!isRootPkg) {
    ...
    if (allImports) {
      ...
      if (
        ...
        impPkgs.includes(`@${group}`) ||
        hasNpmBinCommandEvidence(allImports, pkg)
      ) {
        if (isPkgTypeOnlyImport(allImports, group, name)) {
          compScope = "excluded";
          isTypeOnlyPkg = true;
        } else {
          compScope = "required";
