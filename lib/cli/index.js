...
export async function createJavaBom(path, options) {
  ...
  if (
    ...
    if (pkgList.length) {
      ...
      const repoUrls = extractGradleRepositoryUrls(parallelPropTaskOut);
      const repoUrlsList = Object.values(repoUrls);
      ...
      for (const p of pkgList) {
        ...
        if (!p.externalReferences.find((r) => r.type === "distribution")) {
          ...
          if (!matchedUrl && repoUrlsList.length > 0) {
            const firstRepo = repoUrlsList[0];
            matchedUrl = `${
              firstRepo.endsWith("/") ? firstRepo : `${firstRepo}/`
            }${groupPath}/${p.name}/${p.version}/${p.name}-${p.version}.jar`;
          }
          if (matchedUrl) {
