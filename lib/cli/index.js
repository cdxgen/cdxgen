...
const shouldIncludeNodeModulesDir = (options = {}, baseProjectTypes = []) => {
  ...
  return baseProjectTypes.some((projectType) =>
    PROJECT_TYPE_ALIASES[projectType]?.includes(projectTypes[0]),
  );
};
...
export async function createJarBom(path, options) {
  ...
  let jarFiles;
  let nsMapping = {};
  if (typeof options.includeNodeModulesDir === "undefined") {
    options.includeNodeModulesDir = shouldIncludeNodeModulesDir(options, [
      "jar",
      "war",
      "ear",
    ]);
