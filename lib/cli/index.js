...
export async function createVscodeExtensionBom(path, options) {
  ...
  // Mode 1: Scan for .vsix files in the given directory
  ...
  // Mode 2: Scan for installed extensions in the given directory
  // Check if the path itself is an extensions directory
  const extensionsFromPath = collectInstalledExtensions([
    { name: "", dir: path },
  ]);
  if (extensionsFromPath.length) {
    if (DEBUG_MODE) {
      console.log(
        `Found ${extensionsFromPath.length} installed extension(s) at ${path}`,
      );
    }
    pkgList = pkgList.concat(extensionsFromPath);
    // Deep analysis for each installed extension directory
    await analyzeInstalledExtensionDirs(path, options, pkgList, dependencies);
  }

  // Mode 3: Auto-discover extensions from known IDE locations
...
async function analyzeExtensionDir(extDir, options) {
  ...
  // Otherwise, look for minified JS files and run parseMinJs
  const minJsFiles = getAllFiles(extDir, "**/*min.js", options);
  if (minJsFiles.length) {
    if (DEBUG_MODE) {
      console.log(
        `Found ${minJsFiles.length} minified JS file(s) in extension`,
      );
    }
    for (const f of minJsFiles) {
      const dlist = await parseMinJs(f);
  ...
  // Also look for any package.json files inside bundled sub-packages
  if (hasPackageJson) {
    const pkgJsonFiles = getAllFiles(extDir, "**/package.json", options);
    for (const pj of pkgJsonFiles) {
      const dlist = await parsePkgJson(pj);
      if (dlist?.length) {
        for (const d of dlist) {
  ...
  // Run babel-based source analysis unless disabled
  if (!options.noBabel) {
    try {
      const retData = await findJSImportsExports(extDir, true);
      if (retData.allImports && Object.keys(retData.allImports).length) {
        if (DEBUG_MODE) {
          console.log(
            `Analyzer found ${Object.keys(retData.allImports).length} imports in extension`,
          );
        }
      }
    } catch (_e) {
...
async function analyzeInstalledExtensionDirs(
  ...
  try {
    ...
    entries = readdir(extensionsDir, { withFileTypes: true });
  } catch (_e) {
    return;
  }
  for (const entry of entries) {
    ...
    if (deepResult.pkgList.length) {
      for (const pkg of deepResult.pkgList) {
        pkgList.push(pkg);
      }
    }
    if (deepResult.dependencies.length) {
