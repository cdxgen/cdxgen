...
function createAsarFixture(targetPath, options = {}) {
  ...
  const packedBuffers = [];
  let nextOffset = 0;
  for (const relativeFilePath of collectFixtureFiles(asarFixtureDir)) {
    const absoluteFilePath = join(asarFixtureDir, relativeFilePath);
  ...
  writeFileSync(
    targetPath,
    Buffer.concat([makeSizePickle(headerPickle.length), headerPickle, ...packedBuffers]),
  );
}
  ...
  describe("createAsarBom()", () => {
    it("catalogs ASAR archives, extracts nested npm metadata, and surfaces audit findings", async () => {
      ...
      try {
        ...
        assert.strictEqual(getProp(bomData.parentComponent, "cdx:asar:hasEval"), "true");
        assert.strictEqual(
        ...
        assert.ok(
          findings.some((finding) => finding.ruleId === "ASAR-003"),
    ...
    it("records blocked extraction activity in dry-run mode for ASAR archives", async () => {
      const fixtureRoot = mkdtempSync(join(tmpdir(), "cdxgen-asar-dry-run-"));
      const archivePath = join(fixtureRoot, "app.asar");
      createAsarFixture(archivePath, {
        unpackedPaths: ["native/addon.node"],
      });
      ...
      try {
        ...
        assert.ok(
          activities.some(
            (activity) =>
              activity.capability === "archive-extraction" &&
              activity.archiveKind === "asar" &&
              activity.status === "blocked",
          ),
          "expected blocked ASAR extraction activity in dry-run mode",
        );
      } finally {
        setDryRunMode(false);
        resetRecordedActivities();
