...
`,
  ...
  describe("createBom() MCP inventory support", () => {
    ...
    it("flags disabled setup caches for npm and Python lockfile fixtures", async () => {
      const options = {
        bomAudit: true,
    ...
    it("flags disabled Cargo setup cache when Cargo manifests use git dependencies", async () => {
      const tmpDir = mkdtempSync(join(tmpdir(), "cdxgen-cargo-cache-disable-"));
      mkdirSync(join(tmpDir, ".github", "workflows"), { recursive: true });
      writeFileSync(
        join(tmpDir, "Cargo.toml"),
        `[package]
name = "cargo-cache-disable"
version = "1.0.0"

[dependencies]
git-crate = { git = "https://github.com/acme/git-crate.git" }
`,
      );
      writeFileSync(
        join(tmpDir, ".github", "workflows", "cache-disable.yml"),
        [
          "name: Cargo cache disable",
          "on: push",
          "jobs:",
      ...
      try {
        const bomNSData = await createBom(tmpDir, options);
        const processedBomNSData = postProcess(bomNSData, options, tmpDir);
        const bomJson = processedBomNSData?.bomJson || {};
        ...
        const gitCrateComponent = (bomJson.components || []).find(
          (component) => component.name === "git-crate",
        );
        assert.ok(setupRustComponent, "expected setup-rust workflow component");
        assert.ok(gitCrateComponent, "expected Cargo git dependency component");
        assert.strictEqual(
        ...
        assert.strictEqual(
          getProp(gitCrateComponent, "cdx:cargo:git"),
