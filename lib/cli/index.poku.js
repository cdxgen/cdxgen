...
import { createBom, createChromeExtensionBom } from "./index.js";
...
describe("CLI tests", () => {
  ...
  describe("createBom() cargo cache support", () => {
    it("catalogs cached cargo crate archives via the cargo-cache project type", async () => {
      const tempCargoHome = mkdtempSync(join(tmpdir(), "cdxgen-cargo-cache-"));
      const cacheDir = join(
        tempCargoHome,
        "registry",
        "cache",
      ...
      writeFileSync(join(cacheDir, "serde-1.0.217.crate"), "serde");
      writeFileSync(join(cacheDir, "tokio-1.43.0.crate"), "tokio");
      const originalCargoCacheDir = process.env.CARGO_CACHE_DIR;
      try {
        process.env.CARGO_CACHE_DIR = cacheDir;
        const bomNSData = await createBom(tempCargoHome, {
          deep: false,
          failOnError: true,
          installDeps: false,
          multiProject: false,
        ...
        rmSync(tempCargoHome, { force: true, recursive: true });
      }
    });
  });
});
