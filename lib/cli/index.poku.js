...
function createDryRunPrivateRegistryFixture() {
  const fixtureRoot = mkdtempSync(join(tmpdir(), "cdxgen-dry-run-registry-"));
  writeFileSync(
    join(fixtureRoot, "package.json"),
    JSON.stringify(
      {
        name: "dry-run-private-registry-repotest",
        version: "1.0.0",
        dependencies: {
          "left-pad": "1.3.0",
        },
  ...
  writeFileSync(
    ...
    [
      "@private:registry=https://registry.example.com/npm/",
      "//registry.example.com/npm/:_authToken=secret-token",
      "",
    ].join("\n"),
  );
  writeFileSync(join(fixtureRoot, "private-test.key"), "not-a-real-private-key");
  return fixtureRoot;
}
...
describe("CLI tests", () => {
  ...
  describe("dry-run tracing", () => {
    it("captures sensitive file reads and environment reads for private registry style inputs", () => {
      const fixtureRoot = createDryRunPrivateRegistryFixture();
      try {
        const output = execFileSync(
          process.execPath,
          [join(repoDir, "bin", "cdxgen.js"), "--dry-run", "-t", "js", fixtureRoot, "--no-banner"],
          {
            cwd: repoDir,
            encoding: "utf8",
            env: {
              ...
              SBOM_SIGN_PRIVATE_KEY: join(fixtureRoot, "private-test.key"),
            },
          },
        );
        assert.match(output, /cdxgen dry-run activity summary/);
        assert.match(output, /\.npmrc/);
        assert.match(output, /process\.env:SBOM_SIGN_PRIVATE_KEY/);
      } finally {
