...
describe("CLI tests", () => {
  describe("distribution filters", () => {
    it("keeps npm types while excluding poku tests from shipped artifacts", () => {
      const packageJson = JSON.parse(
        readFileSync(join(repoDir, "package.json"), "utf8"),
      );
      const dockerIgnore = readFileSync(join(repoDir, ".dockerignore"), "utf8");
      const buildStandaloneSh = readFileSync(
        join(repoDir, ".github", "scripts", "build-standalone.sh"),
        "utf8",
      );
      const buildStandalonePs1 = readFileSync(
        join(repoDir, ".github", "scripts", "build-standalone.ps1"),
        "utf8",
      );

      assert.ok(packageJson.files.includes("types/"));
      assert.ok(packageJson.files.includes("!lib/**/*.poku.js"));
      assert.ok(dockerIgnore.includes("**/*.poku.js"));
      assert.ok(!dockerIgnore.includes("!*.md"));
      assert.ok(buildStandaloneSh.includes("*.md"));
      assert.ok(
        buildStandaloneSh.includes('find lib -name "*.poku.js" -delete'),
      );
      assert.ok(buildStandaloneSh.includes("rm -rf types"));
      assert.ok(buildStandalonePs1.includes('"*.md"'));
      assert.ok(buildStandalonePs1.includes('"types"'));
      assert.ok(buildStandalonePs1.includes("Get-ChildItem -Path lib"));
    });
  });
