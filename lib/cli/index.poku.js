...
describe("CLI tests", () => {
  ...
  describe("createBom() cargo cache support", () => {
    ...
    it("creates a Cargo workspace BOM with workflow signals and matching audit findings", async () => {
      ...
      const processedBomNSData = postProcess(bomNSData, options, cargoFixtureDir);
      ...
      const buildHelperComponent = (bomJson.components || []).find(
        (component) => component.name === "build-helper",
      );
      ...
      assert.ok(
        (bomJson.annotations || []).some((annotation) =>
          annotation.text?.includes("INT-012"),
        ),
      );
      assert.ok(
        (bomJson.annotations || []).some((annotation) =>
          annotation.text?.includes("INT-013"),
        ),
      );
