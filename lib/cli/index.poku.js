...
`,
  ...
  describe("createNodejsBom() npm scope and scripts", () => {
    ...
    it("correctly scopes devDependencies vs runtime deps in a Vue app", async () => {
      ...
      // devDependency: vite is a build tool invoked via npm scripts only
      assert.strictEqual(find("", "vite")?.scope, "optional");
      // devDependency: plugin imported only in vite.config.js (excluded from
      // import scanning by IGNORE_FILE_PATTERN), so it stays optional
      assert.strictEqual(find("@vitejs", "plugin-vue")?.scope, "optional");
    });
