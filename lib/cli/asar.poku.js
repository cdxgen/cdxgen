...
describe("createAsarBom()", () => {
  ...
  it("keeps distinct ASAR archives whose paths differ only by case", async () => {
    const fixtureRoot = mkdtempSync(join(tmpdir(), "cdxgen-asar-case-"));
    const lowerArchivePath = join(fixtureRoot, "app.asar");
    const upperArchivePath = join(fixtureRoot, "App.asar");
    createAsarFixture(lowerArchivePath);
    createAsarFixture(upperArchivePath, {
      extraEntries: {
        "package.json": {
          content: JSON.stringify({
    ...
    try {
      ...
      assert.strictEqual(archiveComponents.length, 2);
      assert.ok(
        archiveComponents.some(
          (component) => getProp(component, "SrcFile") === lowerArchivePath,
        ),
      );
      assert.ok(
        archiveComponents.some(
          (component) => getProp(component, "SrcFile") === upperArchivePath,
        ),
      );
    } finally {
      rmSync(fixtureRoot, { force: true, recursive: true });
