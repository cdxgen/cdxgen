...
describe("createAsarBom()", () => {
  ...
  it("keeps distinct nested ASAR archives whose virtual paths differ only by case", async () => {
    const fixtureRoot = mkdtempSync(join(tmpdir(), "cdxgen-asar-case-"));
    const outerArchivePath = join(fixtureRoot, "outer.asar");
    const lowerNestedArchivePath = join(fixtureRoot, "lower-nested.asar");
    const upperNestedArchivePath = join(fixtureRoot, "upper-nested.asar");
    createAsarFixture(lowerNestedArchivePath);
    createAsarFixture(upperNestedArchivePath, {
      extraEntries: {
        "package.json": {
          content: JSON.stringify({
    ...
    createAsarFixture(outerArchivePath, {
      extraEntries: {
        "nested/core.asar": {
          content: readFileSync(lowerNestedArchivePath),
        },
        "nested/Core.asar": {
          content: readFileSync(upperNestedArchivePath),
        },
      },
    });
    try {
      ...
      const nestedArchiveComponents = (bomData.bomJson?.components || []).filter(
        (component) =>
      ...
      assert.ok(
        nestedArchiveComponents.some(
          (component) =>
            ...
            `${outerArchivePath}#/nested/core.asar`,
        ),
      );
      assert.ok(
        nestedArchiveComponents.some(
  ...
  it("recursively scans nested ASAR archives and rewrites nested evidence paths", async () => {
    ...
    try {
      ...
      const nestedArchiveComponent = bomData.bomJson.components.find(
        (component) =>
          getProp(component, "cdx:file:kind") === "asar-archive" &&
          getProp(component, "SrcFile") === `${archivePath}#/nested/core.asar`,
      );
