...
describe("gitClone() hardening tests", () => {
  it("passes core.hooksPath=/dev/null and --template= to git", async () => {
    const spawnStub = sinon.stub().returns({ status: 0, stderr: "" });
    const mkdtempStub = sinon.stub().returns("/tmp/fake-repo");
  ...
  it("does not include GIT_CONFIG_NOGLOBAL in the environment", async () => {
    const spawnStub = sinon.stub().returns({ status: 0, stderr: "" });
    const mkdtempStub = sinon.stub().returns("/tmp/fake-repo");
