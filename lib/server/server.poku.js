...
describe("serverUrl allowlist validation in /sbom route", () => {
  async function createSbomRouteHarness(allowedHosts) {
    const appUseCalls = [];
    const app = {
      use: (...args) => {
    ...
    if (allowedHosts === undefined) {
      delete processEnv.CDXGEN_ALLOWED_HOSTS;
    } else {
      processEnv.CDXGEN_ALLOWED_HOSTS = allowedHosts;
    }
    const processMock = {
      env: processEnv,
      exit: sinon.stub(),
    ...
    const createBomStub = sinon
      .stub()
      .resolves({ bomJson: { bomFormat: "CycloneDX" } });
    const postProcessStub = sinon
      .stub()
      .returns({ bomJson: { bomFormat: "CycloneDX" } });
    const submitBomStub = sinon.stub().resolves();

    const { start } = await esmock("./server.js", {
      connect: { default: sinon.stub().returns(app) },
      "node:http": { default: { createServer: createServerStub } },
      "node:process": { default: processMock },
      "body-parser": {
        default: { json: sinon.stub().returns((_req, _res, next) => next()) },
      },
      compression: {
        default: sinon.stub().returns((_req, _res, next) => next()),
      },
      "../cli/index.js": {
        createBom: createBomStub,
        submitBom: submitBomStub,
      },
      "../helpers/source.js": {
        cleanupSourceDir: sinon.stub(),
        findGitRefForPurlVersion: sinon.stub().returns(undefined),
        getGitAllowProtocol: sinon.stub().returns("https:git:ssh"),
        gitClone: sinon.stub(),
        isAllowedPath: sinon.stub().returns(true),
        isAllowedWinPath: sinon.stub().returns(true),
        maybePurlSource: sinon.stub().returns(false),
        maybeRemotePath: sinon.stub().returns(false),
        PURL_REGISTRY_LOOKUP_WARNING: "warning",
        resolveGitUrlFromPurl: sinon.stub().resolves(undefined),
        resolvePurlSourceDirectory: sinon.stub(),
        sanitizeRemoteUrlForLogs: sinon.stub().returns("sanitized"),
        validateAndRejectGitSource: sinon.stub().returns(null),
        validatePurlSource: sinon.stub().returns(null),
      },
      "../helpers/utils.js": {
        CDXGEN_VERSION: "test-version",
        hasDangerousUnicode: sinon.stub().returns(false),
        isSecureMode: false,
        isWin: false,
      },
      "../stages/postgen/postgen.js": {
        postProcess: postProcessStub,
      },
    });
    ...
    const sbomRouteCall = appUseCalls.find(
      ([route, handler]) => route === "/sbom" && typeof handler === "function",
    );
    assert.ok(sbomRouteCall, "expected /sbom route handler to be registered");
    const sbomHandler = sbomRouteCall[1];
    return {
      createBomStub,
      postProcessStub,
      sbomHandler,
  ...
  it("returns 400 when Dependency-Track serverUrl is invalid", async () => {
    const { createBomStub, postProcessStub, sbomHandler, submitBomStub } =
      await createSbomRouteHarness();

    const req = {
      body: {},
      headers: { host: "localhost" },
      method: "GET",
      url: "/sbom?path=/tmp&serverUrl=not-a-url&apiKey=test-key-fake&projectName=test-project",
    };
    const res = createMockResponse();
    await sbomHandler(req, res);

    assert.strictEqual(res.statusCode, 400);
    const parsed = JSON.parse(res.body);
    checkEqual(parsed.error, "Invalid Server URL");
    sinon.assert.calledOnce(createBomStub);
    sinon.assert.calledOnce(postProcessStub);
    sinon.assert.notCalled(submitBomStub);
  });

  it("returns 403 when Dependency-Track hostname is not allowlisted", async () => {
    const { sbomHandler, submitBomStub } = await createSbomRouteHarness(
      "allowed.example.com",
    );
    ...
    const req = {
      ...
      headers: { host: "localhost" },
      method: "GET",
      url: "/sbom?path=/tmp&serverUrl=https://denied.example.com/api&apiKey=test-key-fake&projectName=test-project",
    };
    const res = createMockResponse();
    await sbomHandler(req, res);
    ...
    const parsed = JSON.parse(res.body);
    checkEqual(parsed.error, "Host Not Allowed");
    sinon.assert.notCalled(submitBomStub);
  });
});
