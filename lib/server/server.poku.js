...
describe("parseQueryString tests", () => {
  ...
  it("parses requested format values", () => {
    const result = parseQueryString({ format: "cyclonedx,spdx" }, {}, {});
    checkEqual(result.format, "cyclonedx,spdx");
  });
});

describe("resolveServerSbomPayload()", () => {
  it("returns CycloneDX payload by default", () => {
    const bomJson = {
      bomFormat: "CycloneDX",
      specVersion: "1.7",
      version: 1,
    };
    const result = resolveServerSbomPayload({ bomJson }, {});
    checkEqual(result, bomJson);
  });

  it("returns SPDX payload when format requests spdx", () => {
    const bomJson = {
      bomFormat: "CycloneDX",
      specVersion: 1.7,
    ...
    const result = resolveServerSbomPayload({ bomJson }, { format: "spdx" });
    checkEqual(Boolean(result?.["@context"]), true);
    checkEqual(Array.isArray(result?.["@graph"]), true);
  });
});
