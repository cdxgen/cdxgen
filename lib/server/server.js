...
const start = (options) => {
  ...
  app.use("/sbom", async (req, res) => {
    ...
    res.end("\n");
    if (cleanup && srcDir && srcDir.startsWith(getTmpDir()) && fs.rm) {
      const dirToRemove = srcDir;
      res.once("finish", () => {
        console.log(`Cleaning up ${dirToRemove}`);
        fs.promises
          .rm(dirToRemove, { recursive: true, force: true })
          .catch((e) => console.warn("cleanup failed:", dirToRemove, e));
      });
  });
};
