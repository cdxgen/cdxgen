...
const start = (options) => {
  ...
  if (!process.env.CDXGEN_GIT_ALLOWED_HOSTS && !process.env.CDXGEN_SERVER_ALLOWED_HOSTS) {
    console.log(
  ...
  app.use("/sbom", async (req, res) => {
    ...
    if (maybeRemotePath(sourcePath)) {
      srcDir = gitClone(sourcePath, reqOptions.gitBranch);
      cleanup = true;
    } else {
      srcDir = sourcePath;
      if (!isAllowedPath(path.resolve(srcDir)) || !isAllowedWinPath(srcDir)) {
