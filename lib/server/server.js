...
export function validateAndRejectGitSource(filePath) {
  ...
  if (maybeRemotePath(filePath)) {
    ...
    const GIT_ALLOW_PROTOCOL =
      process.env.CDXGEN_SERVER_GIT_ALLOW_PROTOCOL || "https:http:git:ssh";
    const allowedSchemes = GIT_ALLOW_PROTOCOL.split(":")
      .filter(Boolean)
      .map((p) => `${p.toLowerCase()}:`);
...
function gitClone(repoUrl, branch = null) {
  ...
  const GIT_ALLOW_PROTOCOL =
    process.env.CDXGEN_SERVER_GIT_ALLOW_PROTOCOL || "https:ssh";
  // See issue #1956
...
const start = (options) => {
  ...
  if (
    ...
    /(ext|fd):/i.test(process.env?.CDXGEN_SERVER_GIT_ALLOW_PROTOCOL)
  ) {
    console.log(
