...
/**
 * Choose the payload to return for /sbom based on the requested format.
 *
 * @param {object} bomNSData BOM payload object
 * @param {object} reqOptions Parsed request options
 * @returns {object|string|undefined} CycloneDX or SPDX payload
 */
export function resolveServerSbomPayload(bomNSData, reqOptions = {}) {
  const normalizedFormats = normalizeOutputFormats(reqOptions?.format);
  const wantsSpdx = normalizedFormats.includes("spdx");
  if (
    wantsSpdx &&
    bomNSData?.bomJson &&
    bomNSData?.bomJson?.bomFormat === "CycloneDX"
  ) {
    bomNSData.spdxJson = convertCycloneDxToSpdx(bomNSData.bomJson, reqOptions);
    if (bomNSData.spdxJson) {
      return bomNSData.spdxJson;
    }
  }
...
const start = (options) => {
  ...
  app.use("/sbom", async (req, res) => {
    ...
    const responsePayload = resolveServerSbomPayload(bomNSData, reqOptions);
    ...
    if (responsePayload) {
      if (
        ...
        res.write(JSON.stringify(responsePayload, null, null));
      }
    }
    res.end("\n");
