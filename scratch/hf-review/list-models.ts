...
export const MODEL_DERIVED_FIELD_TO_API_KEY: Record<
...
export type ModelAdditionalField =
  | Exclude<
      (typeof MODEL_EXPANDABLE_KEYS)[number],
      (typeof MODEL_EXPAND_KEYS)[number]
    >
  | keyof ModelDerivedFields;
...
export async function* listModels<const T extends ModelAdditionalField = never>(
  ...
  let totalToFetch = params?.limit ?? Number.POSITIVE_INFINITY;
  const additionalExpandKeys =
    params?.additionalFields?.map(
      (field) =>
        MODEL_DERIVED_FIELD_TO_API_KEY[field as keyof ModelDerivedFields] ??
        field,
    ) ?? [];
  const search = new URLSearchParams([
    ...Object.entries({
      ...
      ...(params?.search?.task
        ? { pipeline_tag: params.search.task }
        : undefined),
      ...
      ...(params?.search?.inferenceProviders
        ...
        : undefined),
      ...(params?.search?.apps
        ? { apps: params.search.apps.join(",") }
        : undefined),
      ...(params?.sort ? { sort: params.sort } : undefined),
    }),
    ...(params?.search?.tags?.map((tag) => ["filter", tag]) ?? []),
    ...MODEL_EXPAND_KEYS.map(
      (val) => ["expand", val] satisfies [string, string],
    ),
    ...additionalExpandKeys.map(
      (val) => ["expand", val] satisfies [string, string],
    ),
  ]).toString();
  let url: string | undefined =
    `${params?.hubUrl || HUB_URL}/api/models?${search}`;
    ...
    for (const item of items) {
      ...
      if (params?.additionalFields) {
        for (const field of params.additionalFields) {
          if (field === "filePaths") {
            additional.filePaths = (item.siblings ?? []).map(
              (s) => s.rfilename,
            );
          } else if (
            field === "inferenceProviderMapping" &&
            item.inferenceProviderMapping
          ) {
            additional.inferenceProviderMapping =
